...
local function __ArexansHub_Init_Main__()
...
if not ModernV2 then
    ...
    local loaderOk, loaderResult = pcall(function()
        local source = game:HttpGet("https://raw.githubusercontent.com/AREXANS/kumpulan-ui/refs/heads/main/uixans.lua")
    ...
    if loaderOk then
        ...
        warn("[ArexansHub] Vercel mirror failed, trying GitHub fallback:", loaderResult)
        ...
        if fallbackOk then
            ...
            warn("[ArexansHub] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
local UIConfigPath = "ArexansHub_UI_Theme.json"
local XansUI_Solid = false
local XansUI_Color = "Blue"

pcall(function()
    if isfile and readfile and isfile(UIConfigPath) then
        local data = HttpService:JSONDecode(readfile(UIConfigPath))
        XansUI_Solid = data.Solid
        XansUI_Color = data.Color or "Blue"
    end
end)

if XansUI_Color == "Default" then
    XansUI_Color = "Blue"
end
...
if ModernV2 and ModernV2.CreateMenuIcon then
    MenuIcon = ModernV2:CreateMenuIcon({
        Image = "rbxassetid://111967366195505",
        Size = 48,
...
if ModernV2 then
    ...
    Window = ModernV2:Window({
        ...
        Image = "111967366195505",
        -- Tetap aktifkan menu User Settings, tetapi kartu profil di dalamnya
        -- akan dihapus setelah framework selesai membuat UserSettings.
        ShowUser = true,
        Search = true,
        ConfigEnabled = true,
        ...
        Config = {
            ConfigFolder = "ArexansHubViolenceDistrict",
            AutoSaveFile = "AREXANS_VD",
            AutoSave = false,
            AutoLoad = false,
            Overwrite = true,
            Format = "JSON",
    ...
    -- Hapus SEPENUHNYA kartu info profil yang dibuat otomatis oleh ModernV2
    -- di dalam popup User Settings (avatar + nickname + Never).
    -- User Settings / tombol Settings tetap dipertahankan.
    pcall(function()
        local UserSettings = Window.UserSettings
        local UserSettingsRoot = UserSettings and UserSettings.Root
        if UserSettingsRoot then
            for _, child in ipairs(UserSettingsRoot:GetChildren()) do
                if child:IsA("GuiObject") then
                    local hasProfileImage = false
                    local hasProfileText = false
                    local hasNeverText = false
                    ...
                    for _, obj in ipairs(child:GetDescendants()) do
                        ...
                        elseif obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                            ...
                            if text == "Never" or text == "Customize menu" then
                                hasNeverText = true
                            elseif text ~= "" then
                                hasProfileText = true
                            end
                    ...
                    -- Kartu profil bawaan memiliki ImageLabel + nickname + Never/
                    -- Customize menu. Hanya object tersebut yang dihapus.
                    if hasProfileImage and hasProfileText and hasNeverText then
                        child:Destroy()
                    end
                end
            end
    ...
    Window:CreateHomeTab({
        ...
        Content = "ArexansHub Violence District Script",
        WhatsAppInvite = "https://chat.whatsapp.com/JyEGFJCvxfCFRY04m1lYQf?s=cl&p=a&mlu=4&ilr=4",
    ...
    -- Privacy Name & Avatar (User Settings)
    pcall(function()
        if Window.UserSettings then
            Window:AddToggle({
                Default = false,
                Name = "Activate Hide Name (AREXANS) & Privacy Avatar",
                Flag = "Privacy_HideName",
                Callback = function(state)
                    if getgenv().SetHideName then
                        getgenv().SetHideName(state)
                    end
                end
            })
            Window:AddToggle({
                ...
                Flag = "Potato_Grafik",
                Callback = function(state)
                    VD.PotatoMode = state == true
                    AREXANS_SetPotatoGrafik(state == true)
                end
            })
        end
    end)
...
getgenv().VD = getgenv().VD or {
    ...
    PotatoMode            = false,
    PerformanceTick       = 0.12
}

local VD = getgenv().VD

-- =====================================================
-- POTATO GRAFIK / LOW-END HP MODE
-- =====================================================
-- Fast cooperative optimizer: aggressively lowers rendering cost while
-- yielding frequently enough to prevent sudden gameplay freezes.
VD.PotatoMode = (VD.PotatoMode == true)

local AREXANS_PerfClock = {}
local function AREXANS_PerfDue(key, interval)
    local now = os.clock()
    local last = AREXANS_PerfClock[key] or 0
    if now - last < interval then return false end
    AREXANS_PerfClock[key] = now
    return true
end

local AREXANS_Potato = {
    Active = false,
    Busy = false,
    RestoreBusy = false,
    Connections = {},
...
-- Higher throughput than the old 40-object batch, but capped by time.
local POTATO_BATCH_SIZE = 700
local POTATO_FRAME_BUDGET = 0.0040 -- about 4 ms per work slice
local POTATO_MAX_PER_HEARTBEAT = 900

local function AREXANS_PotatoSave(obj, prop)
    if not AREXANS_Potato.Changed[obj] then
        AREXANS_Potato.Changed[obj] = {}
    end
    if AREXANS_Potato.Changed[obj][prop] == nil then
        local ok, value = pcall(function() return obj[prop] end)
        if ok then AREXANS_Potato.Changed[obj][prop] = value end
    end
end

local function AREXANS_PotatoSet(obj, prop, value)
    if not obj or not obj.Parent then return end
    AREXANS_PotatoSave(obj, prop)
    pcall(function() obj[prop] = value end)
end

local function AREXANS_PotatoOptimize(obj)
    if not AREXANS_Potato.Active or not obj then return end

    if obj:IsA("BasePart") then
        AREXANS_PotatoSet(obj, "CastShadow", false)
        AREXANS_PotatoSet(obj, "Reflectance", 0)
        pcall(function() AREXANS_PotatoSet(obj, "Material", Enum.Material.Plastic) end)
        if obj:IsA("MeshPart") then
            pcall(function()
                AREXANS_PotatoSet(obj, "RenderFidelity", Enum.RenderFidelity.Performance)
            end)
        end
    end

    if obj:IsA("Texture") or obj:IsA("Decal") then
        AREXANS_PotatoSet(obj, "Transparency", 1)
    end

    if obj:IsA("SurfaceAppearance") then
        pcall(function() AREXANS_PotatoSet(obj, "Enabled", false) end)
        pcall(function()
            AREXANS_PotatoSet(obj, "ColorMap", "")
            AREXANS_PotatoSet(obj, "MetalnessMap", "")
            AREXANS_PotatoSet(obj, "NormalMap", "")
            AREXANS_PotatoSet(obj, "RoughnessMap", "")
        end)
    end
    ...
    or obj:IsA("Sparkles") then
        AREXANS_PotatoSet(obj, "Enabled", false)
    end

    if obj:IsA("PointLight") or obj:IsA("SpotLight") or obj:IsA("SurfaceLight") then
        AREXANS_PotatoSet(obj, "Enabled", false)
        pcall(function() AREXANS_PotatoSet(obj, "Shadows", false) end)
    end

    if obj:IsA("Atmosphere") then
        AREXANS_PotatoSet(obj, "Density", 0)
        AREXANS_PotatoSet(obj, "Haze", 0)
        AREXANS_PotatoSet(obj, "Glare", 0)
    end

    if obj:IsA("Clouds") then
        AREXANS_PotatoSet(obj, "Cover", 0)
        AREXANS_PotatoSet(obj, "Density", 0)
    end
...
local function AREXANS_PotatoForceRenderQuality()
    -- Executor-compatible best-effort Roblox render quality clamp.
    -- Not all executors expose settings(), so every write is protected.
    pcall(function()
        local gs = settings()
        local rendering = gs and gs.Rendering
        if rendering then
            AREXANS_PotatoSet(rendering, "QualityLevel", Enum.QualityLevel.Level01)
        end
    end)
    pcall(function()
        local us = UserSettings()
        local gameSettings = us and us.GameSettings
        if gameSettings then
            AREXANS_PotatoSet(gameSettings, "SavedQualityLevel", Enum.SavedQualitySetting.QualityLevel1)
        end
    end)
end

local function AREXANS_PotatoDisablePostFX()
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("PostEffect") then
            AREXANS_PotatoSet(child, "Enabled", false)
        end
    end
end

local function AREXANS_PotatoLowLighting()
    AREXANS_PotatoSet(Lighting, "GlobalShadows", false)
    AREXANS_PotatoSet(Lighting, "Brightness", 1)
    pcall(function()
        AREXANS_PotatoSet(Lighting, "EnvironmentDiffuseScale", 0)
        AREXANS_PotatoSet(Lighting, "EnvironmentSpecularScale", 0)
    end)
    pcall(function()
        AREXANS_PotatoSet(Lighting, "Technology", Enum.Technology.Compatibility)
    end)

    -- Remove expensive post-processing immediately.
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("PostEffect") or child:IsA("Atmosphere") or child:IsA("Clouds") then
            AREXANS_PotatoOptimize(child)
        end
    end
end

local function AREXANS_PotatoLowTerrain()
    local Terrain = Workspace:FindFirstChildOfClass("Terrain")
    if not Terrain then return end

    -- Critical for Terrain grass/decorations: disable them immediately.
    AREXANS_PotatoSet(Terrain, "Decoration", false)
    AREXANS_PotatoSet(Terrain, "WaterWaveSize", 0)
    AREXANS_PotatoSet(Terrain, "WaterWaveSpeed", 0)
    AREXANS_PotatoSet(Terrain, "WaterReflectance", 0)
    pcall(function() AREXANS_PotatoSet(Terrain, "WaterTransparency", 1) end)
end

local function AREXANS_PotatoProcessMap()
    if AREXANS_Potato.Busy then return end
    AREXANS_Potato.Busy = true

    local objects = Workspace:GetDescendants()
    local total = #objects
    local index = 1

    while AREXANS_Potato.Active and index <= total do
        local sliceStart = os.clock()
        local processed = 0

        while AREXANS_Potato.Active and index <= total and processed < POTATO_BATCH_SIZE do
            AREXANS_PotatoOptimize(objects[index])
            index += 1
            processed += 1
            if processed >= 20 and os.clock() - sliceStart >= POTATO_FRAME_BUDGET then
                break
            end
        end

        -- Yield every slice so movement/input/rendering stay responsive.
        if AREXANS_Potato.Active then
            RunService.Heartbeat:Wait()
        end
    end

    AREXANS_Potato.Busy = false
end

local function AREXANS_PotatoProcessNewObjects()
    while AREXANS_Potato.Active do
        local sliceStart = os.clock()
        local processed = 0

        while AREXANS_Potato.Active
        and AREXANS_Potato.QueueHead <= #AREXANS_Potato.NewObjectQueue
        and processed < POTATO_BATCH_SIZE do
            local obj = AREXANS_Potato.NewObjectQueue[AREXANS_Potato.QueueHead]
            AREXANS_Potato.NewObjectQueue[AREXANS_Potato.QueueHead] = nil
            AREXANS_Potato.QueueHead += 1
            if obj then AREXANS_PotatoOptimize(obj) end
            processed += 1
            if processed >= 20 and os.clock() - sliceStart >= POTATO_FRAME_BUDGET then
                break
            end
        ...
        if AREXANS_Potato.QueueHead > 200
        and AREXANS_Potato.QueueHead > (#AREXANS_Potato.NewObjectQueue / 2) then
            local fresh = {}
            for i = AREXANS_Potato.QueueHead, #AREXANS_Potato.NewObjectQueue do
                fresh[#fresh + 1] = AREXANS_Potato.NewObjectQueue[i]
            end
            AREXANS_Potato.NewObjectQueue = fresh
            AREXANS_Potato.QueueHead = 1
        end
...
local function AREXANS_PotatoStartHandler()
    if AREXANS_Potato.Connections.NewObject then return end
    AREXANS_Potato.Connections.NewObject = Workspace.DescendantAdded:Connect(function(obj)
        if AREXANS_Potato.Active then
            AREXANS_Potato.NewObjectQueue[#AREXANS_Potato.NewObjectQueue + 1] = obj
        end
    end)
    AREXANS_Potato.Connections.LightingObject = Lighting.DescendantAdded:Connect(function(obj)
        if AREXANS_Potato.Active then
            task.defer(function()
                if AREXANS_Potato.Active then AREXANS_PotatoOptimize(obj) end
            end)
        end
    end)
end

local function AREXANS_PotatoEnable()
    if AREXANS_Potato.Active then return end
    AREXANS_Potato.Active = true
    VD.PotatoMode = true

    -- Global lighting and Terrain grass are changed immediately.
    AREXANS_PotatoLowLighting()
    AREXANS_PotatoLowTerrain()
    AREXANS_PotatoStartHandler()

    -- Map scan is cooperative and runs independently from the new-object queue.
    task.spawn(AREXANS_PotatoProcessMap)
    task.spawn(AREXANS_PotatoProcessNewObjects)
end

local function AREXANS_PotatoDisable()
    if not AREXANS_Potato.Active and next(AREXANS_Potato.Changed) == nil then
        VD.PotatoMode = false
        return
    end

    AREXANS_Potato.Active = false
    VD.PotatoMode = false

    for name, connection in pairs(AREXANS_Potato.Connections) do
        pcall(function() connection:Disconnect() end)
        AREXANS_Potato.Connections[name] = nil
    end

    if AREXANS_Potato.RestoreBusy then return end
    AREXANS_Potato.RestoreBusy = true

    task.spawn(function()
        local count = 0
        local sliceStart = os.clock()

        for obj, properties in pairs(AREXANS_Potato.Changed) do
            if obj and obj.Parent then
                for prop, value in pairs(properties) do
                    pcall(function() obj[prop] = value end)
                    count += 1
                    if count >= 20 and os.clock() - sliceStart >= POTATO_FRAME_BUDGET then
                        count = 0
                        sliceStart = os.clock()
                        RunService.Heartbeat:Wait()
                    end
        ...
        table.clear(AREXANS_Potato.Changed)
        table.clear(AREXANS_Potato.NewObjectQueue)
        AREXANS_Potato.QueueHead = 1
        AREXANS_Potato.RestoreBusy = false
    end)
end

function AREXANS_SetPotatoGrafik(state)
    if state then
        AREXANS_PotatoEnable()
    else
        AREXANS_PotatoDisable()
    end
end

-- =====================================================
-- PRIVACY NAME & AVATAR (Streamer Privacy)
-- Ported from vd6.lua
-- =====================================================
do
    local PRIVACY_TARGET_NAME = "AREXANS"
    local PRIVACY_TARGET_IMAGE = "rbxassetid://111967366195505"
    local PrivacyOriginalUI = setmetatable({}, {__mode = "k"})
    local PrivacyConnections = {}
    local PrivacyHookInstalled = false

    local function PrivacyGetGuiParent()
    ...
    local function PrivacyRestoreUI()
        for instance, props in pairs(PrivacyOriginalUI) do
            pcall(function()
                if props.Text and (instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox")) then
                    instance.Text = props.Text
                end
                if props.Image and (instance:IsA("ImageLabel") or instance:IsA("ImageButton")) then
                    instance.Image = props.Image
                end
            end)
        end
        table.clear(PrivacyOriginalUI)
    end

    local function PrivacyFilterPlayerName(text)
        if not text or text == "" or text == PRIVACY_TARGET_NAME then return false, text end
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if pName ~= "" and string.find(lowerText, string.lower(pName), 1, true) then
                shouldReplace = true
                newText = string.gsub(newText, pName, PRIVACY_TARGET_NAME)
                newText = string.gsub(newText, string.lower(pName), PRIVACY_TARGET_NAME)
            end
            if dName ~= "" and string.find(lowerText, string.lower(dName), 1, true) then
                shouldReplace = true
                newText = string.gsub(newText, dName, PRIVACY_TARGET_NAME)
                newText = string.gsub(newText, string.lower(dName), PRIVACY_TARGET_NAME)
            end
        end
        return shouldReplace, newText
    end

    local function PrivacyIsPlayerAvatarIcon(instance, imageStr)
        local imgLower = string.lower(tostring(imageStr or ""))
        if imgLower == "" or imgLower == string.lower(PRIVACY_TARGET_IMAGE) then return false end

        if string.find(imgLower, "rbxthumb://") and (string.find(imgLower, "avatar") or string.find(imgLower, "headshot")) then
            return true
        end

        local instName = string.lower(instance.Name)
        if instName == "avatar" or instName == "playericon" or instName == "profile" or instName == "headshot" then
            local isSmall = true
            pcall(function()
                if instance:IsA("GuiObject") and (instance.AbsoluteSize.X > 200 or instance.AbsoluteSize.Y > 200) then
                    isSmall = false
                end
            end)
            if isSmall then return true end
        end

        return false
    end

    local function PrivacyProcessText(instance)
        if not instance or typeof(instance) ~= "Instance" then return end
        pcall(function()
            if instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
                local currentText = instance.Text
                local shouldReplace, newText = PrivacyFilterPlayerName(currentText)
                if shouldReplace then
                    if not PrivacyOriginalUI[instance] then PrivacyOriginalUI[instance] = {} end
                    if not PrivacyOriginalUI[instance].Text then PrivacyOriginalUI[instance].Text = currentText end
    ...
    local function PrivacyProcessImage(instance)
        if not instance or typeof(instance) ~= "Instance" then return end
        pcall(function()
            if instance:IsA("ImageLabel") or instance:IsA("ImageButton") then
                local currentImage = instance.Image
                if PrivacyIsPlayerAvatarIcon(instance, currentImage) then
                    if not PrivacyOriginalUI[instance] then PrivacyOriginalUI[instance] = {} end
                    if not PrivacyOriginalUI[instance].Image then PrivacyOriginalUI[instance].Image = currentImage end
                    instance.Image = PRIVACY_TARGET_IMAGE
                end
            end
        end)
    end

    local function PrivacySweep()
        task.spawn(function()
            local function scan(parentGui)
                if not parentGui or typeof(parentGui) ~= "Instance" then return end
                for _, instance in ipairs(parentGui:GetDescendants()) do
                    PrivacyProcessText(instance)
                    PrivacyProcessImage(instance)
                end
            end
    ...
    local function PrivacySetupListeners()
        for _, conn in ipairs(PrivacyConnections) do pcall(function() conn:Disconnect() end) end
        table.clear(PrivacyConnections)

        local function attach(instance)
            if not VD.HideName then return end
            PrivacyProcessText(instance)
            PrivacyProcessImage(instance)

            if instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
                table.insert(PrivacyConnections, instance:GetPropertyChangedSignal("Text"):Connect(function()
                    if VD.HideName then PrivacyProcessText(instance) end
                end))
            elseif instance:IsA("ImageLabel") or instance:IsA("ImageButton") then
                table.insert(PrivacyConnections, instance:GetPropertyChangedSignal("Image"):Connect(function()
                    if VD.HideName then PrivacyProcessImage(instance) end
                end))
            end
        end

        local function listen(parentGui)
            if not parentGui or typeof(parentGui) ~= "Instance" then return end
            for _, descendant in ipairs(parentGui:GetDescendants()) do attach(descendant) end
            table.insert(PrivacyConnections, parentGui.DescendantAdded:Connect(function(descendant)
                if VD.HideName then attach(descendant) end
            end))
        end

        local playerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui")
        if playerGui then listen(playerGui) end
        pcall(function() listen(game:GetService("CoreGui")) end)
    end

    local function PrivacySetupNameHook()
        if PrivacyHookInstalled then return end
        PrivacyHookInstalled = true

        local oldNewIndex
        if hookmetamethod and newcclosure then
            oldNewIndex = hookmetamethod(game, "__newindex", newcclosure(function(t, k, v)
                if VD.HideName and type(v) == "string" then
                    if k == "Text" then
                        local shouldReplace, newText = PrivacyFilterPlayerName(v)
                        if shouldReplace then
                            if not PrivacyOriginalUI[t] then PrivacyOriginalUI[t] = {} end
                            PrivacyOriginalUI[t].Text = v
                            return oldNewIndex(t, k, newText)
                        end
                    elseif k == "Image" and typeof(t) == "Instance" and PrivacyIsPlayerAvatarIcon(t, v) then
                        if not PrivacyOriginalUI[t] then PrivacyOriginalUI[t] = {} end
                        PrivacyOriginalUI[t].Image = v
                        return oldNewIndex(t, k, PRIVACY_TARGET_IMAGE)
                    end
                end
    ...
    getgenv().SetHideName = function(state)
        VD.HideName = state == true
        if VD.HideName then
            PrivacySetupListeners()
            PrivacySweep()
        else
            for _, conn in ipairs(PrivacyConnections) do pcall(function() conn:Disconnect() end) end
            table.clear(PrivacyConnections)
    ...
    pcall(PrivacySetupNameHook)
    task.spawn(function()
        while task.wait(2) do
            if VD.HideName then PrivacySweep() end
        end
    end)
end
...
function VD_UpdateCrosshair()
    ...
    CrosshairGui.Name = "AREXANS_Crosshair"
...
function GetSafeChamsFolder()
    ...
    local f = pg:FindFirstChild("AREXANS_WorkspaceChams")
    if not f then
        f = Instance.new("Folder")
        f.Name = "AREXANS_WorkspaceChams"
        f.Parent = pg
    end
    VD_ChamsFolder = f
    return f
...
local ConfigFolderName = "ArexansHub_VD"
local HttpService = game:GetService("HttpService")
...
function AREXANS_SaveConfig(name)
...
local VD_To_Flag = {
    ...
    VIS_XanstKiller = "Xanst Killer Display",
    ...
    PotatoMode = "Potato_Grafik",
}

function AREXANS_LoadConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
    if not name or name == "" then name = "Default" end
    local path = ConfigFolderName .. "/" .. name .. ".json"
    pcall(function()
        if readfile and isfile and isfile(path) then
            ...
            if getgenv().AREXANS_SyncLoadedFeatures then pcall(getgenv().AREXANS_SyncLoadedFeatures) end
        end
    end)
end

function AREXANS_DeleteConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
...
function VD_ApplyWeather(themeName)
    local theme = AREXANS_WeatherPresets[themeName]
    if not theme then theme = AREXANS_WeatherPresets["Default"] end
    ...
    if theme.Particle then
        ...
        pe.Parent = anchor
        if VD.PotatoMode and themeName == "Heavy Rain (Storm)" then
            pcall(function() pe.Rate = math.min(pe.Rate, 900) end)
        end
        if themeName == "Heavy Rain (Storm)" then
            ...
            nearRain.Rate = VD.PotatoMode and 650 or 1800
            ...
            rainSheet.Rate = VD.PotatoMode and 300 or 650
...
-- CHARACTER REFS
-- =====================================================
-- Character, Humanoid, and Root are declared at the top of __ArexansHub_Init_Main__
...
-- =====================================================
local AREXANS_WorldReg
getgenv().AREXANS_oldNamecall = nil

function setupAntiFail()
    if getgenv().AREXANS_AntiFailHooked then return end
    getgenv().AREXANS_AntiFailHooked = true
    task.spawn(function()
...
-- → infinite recursion → stack overflow → force close.
            ...
            _genv.AREXANS_oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                if VD.KILLER_SilentAimFlask and method == "FireServer" then
                    ...
                    if ok and name == "ThrowFlask" then
                        ...
                        if closest then
                            ...
                            return _genv.AREXANS_oldNamecall(self, unpack(args))
                        end
                    end
                end

                if method == "FireServer" and not checkcaller() then
                    local flashRemote = _genv.AREXANS_FlashlightActivateRemote
                    if flashRemote and self == flashRemote and _genv.AREXANS_SetFlashlightAimActive then
                        local args = { ... }
                        pcall(_genv.AREXANS_SetFlashlightAimActive, args[2] == true, args[1])
                    end
                end

                if _genv.AREXANS_oldNamecall then
                    return _genv.AREXANS_oldNamecall(self, ...)
                end
            end)
...
-- =====================================================
getgenv().AREXANS_fpWasSet = false
getgenv().AREXANS_fpOriginal = nil

function RestoreFirstPersonCamera()
    if not getgenv().AREXANS_fpWasSet then return end
    getgenv().AREXANS_fpWasSet = false

    pcall(function()
        if getgenv().AREXANS_fpOriginal then
            LocalPlayer.CameraMode = getgenv().AREXANS_fpOriginal.CameraMode or Enum.CameraMode.Classic
            LocalPlayer.CameraMaxZoomDistance = getgenv().AREXANS_fpOriginal.CameraMaxZoomDistance or 128
            LocalPlayer.CameraMinZoomDistance = getgenv().AREXANS_fpOriginal.CameraMinZoomDistance or 0.5
        else
...
task.spawn(function()
    while not VD.Destroyed do
        ...
        if VD.SURV_FirstPerson then
            ...
            if isSurvivor then
                if not getgenv().AREXANS_fpWasSet then
                    getgenv().AREXANS_fpOriginal = {
                ...
                getgenv().AREXANS_fpWasSet = true
            elseif getgenv().AREXANS_fpWasSet then
...
-- Kontrol ditambahkan ke tab Visual lewat getgenv().AREXANS_AddVisualESPControls.
-- =====================================================
do
    if getgenv().AREXANS_VD_VisualESP_Cleanup then
        pcall(getgenv().AREXANS_VD_VisualESP_Cleanup)
    end

    local LP = LocalPlayer
    local AREXANS_Dead = false
    local AREXANS_ControlsAdded = false

    local AREXANS_ESPState = {
    ...
    getgenv().AREXANS_VD_VisualESP_State = AREXANS_ESPState
    ...
    local AREXANS_MapAdd, AREXANS_MapRem = {}, {}
    local AREXANS_PlayerConns = {}
    local AREXANS_Connections = {}
    local AREXANS_PalletState = setmetatable({}, { __mode = "k" })
    local AREXANS_WindowState = setmetatable({}, { __mode = "k" })
    local AREXANS_InstanceIds = setmetatable({}, { __mode = "k" })
    local AREXANS_XanstId = 0
    local AREXANS_PlayerLoopThread = nil
    local AREXANS_WorldLoopThread = nil
    local AREXANS_ESPFolder = nil

    local AREXANS_DisplayNames = {
        ["Motion Tracker"] = true,
        ["Gate"] = true,
        ["Flashlight"] = true,
        ["Bandage"] = true,
    ...
    local function AREXANS_Alive(inst)
        if not inst then return false end
        local ok, parent = pcall(function() return inst.Parent end)
        return ok and parent ~= nil
    end

    local function AREXANS_Clamp(n, lo, hi)
        n = tonumber(n) or lo
    ...
    local function AREXANS_PlayerKey(player)
    ...
    local function AREXANS_EspId(inst)
        if not inst then return "nil" end
        local id = AREXANS_InstanceIds[inst]
        if id then return id end
        AREXANS_XanstId = AREXANS_XanstId + 1
        id = tostring(AREXANS_XanstId)
        AREXANS_InstanceIds[inst] = id
        return id
    end

    local function AREXANS_GetESPParent()
    ...
    local function AREXANS_GetESPFolder()
        if AREXANS_ESPFolder and AREXANS_ESPFolder.Parent then
            return AREXANS_ESPFolder
        end

        local parent = AREXANS_GetESPParent()
        local old = parent:FindFirstChild("ArexansHub_VisualESP") or parent:FindFirstChild("ZiaanHub_ESP")
        if old then old:Destroy() end

        local folder = Instance.new("Folder")
        folder.Name = "ArexansHub_VisualESP"
        folder.Parent = parent
        AREXANS_ESPFolder = folder
        return folder
    end

    local function AREXANS_ClearPrefix(prefix, keepName)
        local folder = AREXANS_GetESPFolder()
    ...
    local function AREXANS_SafeNotify(title, content, duration)
        pcall(function()
    ...
    local function AREXANS_ValidPart(part)
        return part and AREXANS_Alive(part) and part:IsA("BasePart")
    end

    local function AREXANS_FirstBasePart(inst)
        if not AREXANS_Alive(inst) then return nil end
        if inst:IsA("BasePart") then return inst end
        if inst:IsA("Model") then
            if inst.PrimaryPart and inst.PrimaryPart:IsA("BasePart") and AREXANS_Alive(inst.PrimaryPart) then
                return inst.PrimaryPart
            end
            local part = inst:FindFirstChildWhichIsA("BasePart", true)
            if AREXANS_ValidPart(part) then return part end
        end
        if inst:IsA("Tool") then
            local handle = inst:FindFirstChild("Handle") or inst:FindFirstChildWhichIsA("BasePart")
            if AREXANS_ValidPart(handle) then return handle end
        end
        return nil
    end

    local function AREXANS_GetRole(player)
    ...
    local function AREXANS_PlayerRoleEnabled(player)
        local role = AREXANS_GetRole(player)
        if role == "Killer" then return AREXANS_ESPState.KillerESP end
        if role == "Spectator" then return AREXANS_ESPState.SpectatorESP end
        return AREXANS_ESPState.SurvivorESP
    end

    local function AREXANS_PlayerColor(player)
        local role = AREXANS_GetRole(player)
        if role == "Killer" then return AREXANS_ESPState.KillerColor end
        if role == "Spectator" then return AREXANS_ESPState.SpectatorColor end
        return AREXANS_ESPState.SurvivorColor
    end

    getgenv().AREXANS_VD_VisualESP_HasPlayerText = function(player)
        if not player or player == LP then return false end
        return AREXANS_ESPState.PlayerMasterESP
            and AREXANS_PlayerRoleEnabled(player)
            and (AREXANS_ESPState.Nametags or AREXANS_ESPState.DistanceESP)
    end

    local function AREXANS_EnsureHighlight(name, adornee, color, isPlayer)
        if not (adornee and AREXANS_Alive(adornee)) then return nil end
        local folder = AREXANS_GetESPFolder()
        AREXANS_ClearPrefix(name, name)
        ...
        if isPlayer then
            hl.FillTransparency = AREXANS_ESPState.ESPFillTransparency
            hl.OutlineTransparency = AREXANS_ESPState.ESPOutlineTransparency
        else
    ...
    local function AREXANS_DestroyChild(name)
        local folder = AREXANS_GetESPFolder()
        local child = folder:FindFirstChild(name)
        if child then child:Destroy() end
    end

    local function AREXANS_ClearPlayerESP(player)
        if not player or player == LP then return end
        local key = AREXANS_PlayerKey(player)
        AREXANS_DestroyChild("AREXANS_PlayerHL_" .. key)
        AREXANS_DestroyChild("AREXANS_PlayerTag_" .. key)
        AREXANS_DestroyChild("AREXANS_PlayerItem_" .. key)
        AREXANS_DestroyChild("AREXANS_StunHL_" .. key)
        AREXANS_DestroyChild("AREXANS_StunTag_" .. key)
    end

    local function AREXANS_ClearAllPlayerESP()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LP then
                AREXANS_ClearPlayerESP(player)
            end
        end
    end

    local function AREXANS_GetSurvivorItem(player)
        local character = player.Character
        if not character then return nil end
        for _, obj in ipairs(character:GetDescendants()) do
            if obj:IsA("Tool") or obj:IsA("Accessory") or obj:IsA("Model") then
                if AREXANS_DisplayNames[obj.Name] then
                    return obj.Name
                end
            end
        end
    ...
    local function AREXANS_GetItemImageId(itemName)
    ...
    local function AREXANS_SetBillboardLine(parent, index, count, data)
        ...
        label.TextSize = AREXANS_ESPState.ESPTextSize
        label.TextColor3 = data.Color
        label.Text = data.Text
    end

    local function AREXANS_PruneBillboardLines(parent, count)
    ...
    local function AREXANS_UpdatePlayerTag(player, character, head, color)
        local key = AREXANS_PlayerKey(player)
        local tagName = "AREXANS_PlayerTag_" .. key
        local folder = AREXANS_GetESPFolder()
        AREXANS_ClearPrefix("AREXANS_PlayerTag_" .. key, tagName)

        if not AREXANS_ValidPart(head) then
            AREXANS_DestroyChild(tagName)
            return
        end
        ...
        local distanceText = ""
        if AREXANS_ESPState.DistanceESP and root and targetRoot then
            distanceText = "[" .. tostring(math.floor((root.Position - targetRoot.Position).Magnitude)) .. "m]"
        end

        local nameText = AREXANS_ESPState.Nametags and player.Name or ""
        local mainLine = ""
        ...
        for i, data in ipairs(lines) do
            AREXANS_SetBillboardLine(tag, i, #lines, data)
        end
        AREXANS_PruneBillboardLines(tag, #lines)
    end

    local function AREXANS_UpdatePlayerItemIcon(player, torso)
        local key = AREXANS_PlayerKey(player)
        local iconName = "AREXANS_PlayerItem_" .. key
        local folder = AREXANS_GetESPFolder()
        AREXANS_ClearPrefix("AREXANS_PlayerItem_" .. key, iconName)

        if not AREXANS_ValidPart(torso) then
            AREXANS_DestroyChild(iconName)
            return
        end

        local itemName = AREXANS_GetSurvivorItem(player)
        local imageId = itemName and AREXANS_GetItemImageId(itemName) or nil
        if not imageId then
            AREXANS_DestroyChild(iconName)
            return
        end
    ...
    local function AREXANS_IsPlayerStunned(player, character)
        if not player or not character then return false end
        local attrNames = { "IsStunned", "isStunned", "Stunned", "stunned", "IsStun", "isStun" }
        for _, name in ipairs(attrNames) do
            local ok, value = pcall(function() return character:GetAttribute(name) end)
            if ok and value == true then return true end
        end

        local state = character:GetAttribute("State")
        if type(state) == "string" and state:lower():find("stun", 1, true) then
            return true
        end
        ...
        if hum then
            ...
            for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                ...
                local name = (track.Name or "") .. " " .. (anim and anim.Name or "")
                if name:lower():find("stun", 1, true) then
                    return true
                end
            end
        end
    ...
    local function AREXANS_UpdateStunESP(player, character)
        if not player or player == LP then return end
        local key = AREXANS_PlayerKey(player)
        local killer = AREXANS_GetRole(player) == "Killer"
        if not AREXANS_ESPState.KillerStunnedESP or not killer or not AREXANS_Alive(character) then
            AREXANS_DestroyChild("AREXANS_StunHL_" .. key)
            AREXANS_DestroyChild("AREXANS_StunTag_" .. key)
            return
        end

        if not AREXANS_IsPlayerStunned(player, character) then
            AREXANS_DestroyChild("AREXANS_StunHL_" .. key)
            AREXANS_DestroyChild("AREXANS_StunTag_" .. key)
            return
        end
        ...
        local folder = AREXANS_GetESPFolder()
        local hl = folder:FindFirstChild("AREXANS_StunHL_" .. key)
        if not hl then
            hl = Instance.new("Highlight")
            hl.Name = "AREXANS_StunHL_" .. key
            hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
            hl.FillColor = Color3.fromRGB(255, 210, 40)
            hl.OutlineColor = Color3.fromRGB(255, 245, 130)
        ...
        if head then
            local tag = folder:FindFirstChild("AREXANS_StunTag_" .. key)
            if not tag then
                tag = Instance.new("BillboardGui")
                tag.Name = "AREXANS_StunTag_" .. key
                tag.AlwaysOnTop = true
                tag.LightInfluence = 0
                tag.MaxDistance = 0
    ...
    local AREXANS_ApplyPlayerESP
    AREXANS_ApplyPlayerESP = function(player)
        if AREXANS_Dead or not player or player == LP then return end
        local character = player.Character
        if not (character and AREXANS_Alive(character)) then
            AREXANS_ClearPlayerESP(player)
            return
        end

        AREXANS_UpdateStunESP(player, character)

        local key = AREXANS_PlayerKey(player)
        local enabled = AREXANS_ESPState.PlayerMasterESP and AREXANS_PlayerRoleEnabled(player)
        if not enabled then
            AREXANS_ClearPlayerESP(player)
            return
        end

        local color = AREXANS_PlayerColor(player)
        ...
        AREXANS_EnsureHighlight("AREXANS_PlayerHL_" .. key, character, color, true)
        AREXANS_UpdatePlayerTag(player, character, head, color)

        if AREXANS_GetRole(player) == "Survivor" and AREXANS_ESPState.SurvivorItemsESP then
            AREXANS_UpdatePlayerItemIcon(player, torso)
        else
            AREXANS_DestroyChild("AREXANS_PlayerItem_" .. key)
        end
    end

    local function AREXANS_RefreshAllPlayers()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LP then
                pcall(AREXANS_ApplyPlayerESP, player)
            end
        end
    end

    local function AREXANS_StartPlayerLoop()
        if AREXANS_PlayerLoopThread then return end
        AREXANS_PlayerLoopThread = task.spawn(function()
            while not AREXANS_Dead and AREXANS_ESPState.PlayerMasterESP do
                AREXANS_RefreshAllPlayers()
                task.wait(0.25)
            end
            AREXANS_PlayerLoopThread = nil
        end)
    end

    local AREXANS_StunLoopThread
    local function AREXANS_StartStunLoop()
        if AREXANS_StunLoopThread then return end
        AREXANS_StunLoopThread = task.spawn(function()
            while not AREXANS_Dead and AREXANS_ESPState.KillerStunnedESP do
                for _, player in ipairs(Players:GetPlayers()) do
                    if player ~= LP and player.Character then
                        pcall(AREXANS_UpdateStunESP, player, player.Character)
                    end
                end
                task.wait(0.08)
            end
            AREXANS_StunLoopThread = nil
        end)
    end

    local AREXANS_StunLoopOriginal = AREXANS_RefreshAllPlayers
    AREXANS_RefreshAllPlayers = function(...)
        AREXANS_StunLoopOriginal(...)
        if AREXANS_ESPState.KillerStunnedESP then AREXANS_StartStunLoop() end
    end

    local function AREXANS_WatchPlayer(player)
        if player == LP then return end
        if AREXANS_PlayerConns[player] then
            for _, conn in ipairs(AREXANS_PlayerConns[player]) do
                if conn then pcall(function() conn:Disconnect() end) end
            end
        end

        AREXANS_PlayerConns[player] = {}
        table.insert(AREXANS_PlayerConns[player], player.CharacterAdded:Connect(function(char)
            AREXANS_ClearPlayerESP(player)
            task.delay(0.15, function()
                if not AREXANS_Dead then pcall(AREXANS_ApplyPlayerESP, player) end
            end)
        end))
        table.insert(AREXANS_PlayerConns[player], player.CharacterRemoving:Connect(function()
            AREXANS_ClearPlayerESP(player)
        end))
        table.insert(AREXANS_PlayerConns[player], player:GetPropertyChangedSignal("Team"):Connect(function()
            AREXANS_ClearPlayerESP(player)
            pcall(AREXANS_ApplyPlayerESP, player)
        end))
    ...
    local function AREXANS_UnwatchPlayer(player)
        AREXANS_ClearPlayerESP(player)
        if AREXANS_PlayerConns[player] then
            for _, conn in ipairs(AREXANS_PlayerConns[player]) do
                if conn then pcall(function() conn:Disconnect() end) end
        ...
        AREXANS_PlayerConns[player] = nil
    end

    local function AREXANS_PickWorldPart(model, cat)
        if not (model and AREXANS_Alive(model)) then return nil end
        if cat == "Generator" then
            local hitbox = model:FindFirstChild("HitBox", true) or model:FindFirstChild("GeneratorPoint", true)
            if AREXANS_ValidPart(hitbox) then return hitbox end
        ...
        elseif cat == "Window" then
            ...
            if AREXANS_ValidPart(vault) then return vault end
        elseif cat == "SCPZombie" then
            local root = model:FindFirstChild("HumanoidRootPart", true)
            if AREXANS_ValidPart(root) then return root end
            local torso = model:FindFirstChild("UpperTorso", true) or model:FindFirstChild("Torso", true)
            if AREXANS_ValidPart(torso) then return torso end
            return nil
        end
        return AREXANS_FirstBasePart(model)
    end

    local function AREXANS_GeneratorLabel(model)
        local pct = tonumber(model:GetAttribute("RepairProgress")) or 0
        if pct >= 0 and pct <= 1.001 then pct = pct * 100 end
        pct = AREXANS_Clamp(pct, 0, 100)
        ...
        local hue = AREXANS_Clamp((pct / 100) * 0.33, 0, 0.33)
        return table.concat(parts, " "), Color3.fromHSV(hue, 1, 1)
    end

    local function AREXANS_HasBasePart(model)
        if not (model and AREXANS_Alive(model)) then return false end
        return model:FindFirstChildWhichIsA("BasePart", true) ~= nil
    end

    local function AREXANS_IsPalletGone(model)
        if not AREXANS_Alive(model) then return true end
        if not model:IsDescendantOf(Workspace) then return true end
        if AREXANS_PalletState[model] == "DEST" then return true end
        local ok, destroyed = pcall(function() return model:GetAttribute("Destroyed") end)
        if ok and destroyed == true then return true end
        return not AREXANS_HasBasePart(model)
    end

    local function AREXANS_WorldKey(cat, model)
        return "AREXANS_World_" .. cat .. "_" .. AREXANS_EspId(model)
    end

    local function AREXANS_ClearWorldVisual(cat, model)
        if not model then return end
        AREXANS_DestroyChild(AREXANS_WorldKey(cat, model) .. "_HL")
        AREXANS_DestroyChild(AREXANS_WorldKey(cat, model) .. "_Tag")
    end

    local function AREXANS_RemoveWorldEntry(cat, model)
        if not AREXANS_WorldReg[cat] or not AREXANS_WorldReg[cat][model] then return end
        AREXANS_ClearWorldVisual(cat, model)
        AREXANS_WorldReg[cat][model] = nil
    end

    local function AREXANS_EnsureWorldEntry(cat, model)
        if not AREXANS_Alive(model) or not AREXANS_WorldReg[cat] or AREXANS_WorldReg[cat][model] then return end
        if cat == "Palletwrong" and AREXANS_IsPalletGone(model) then return end
        local part = AREXANS_PickWorldPart(model, cat)
        if not AREXANS_ValidPart(part) then return end
        AREXANS_WorldReg[cat][model] = { part = part }
    end

    local function AREXANS_RegisterWorldDescendant(obj)
        if not AREXANS_Alive(obj) then return end
        local validCats = { Generator = true, Hook = true, Gate = true, Window = true, Palletwrong = true }

        if obj:IsA("Model") then
            if validCats[obj.Name] then
                AREXANS_EnsureWorldEntry(obj.Name, obj)
                return
            end
            local lower = obj.Name:lower()
            if lower:find("scp") or lower:find("zombie") then
                AREXANS_EnsureWorldEntry("SCPZombie", obj)
            end
            return
        end
            ...
            while parent and parent ~= Workspace do
                if parent:IsA("Model") then
                    if validCats[parent.Name] then
                        AREXANS_EnsureWorldEntry(parent.Name, parent)
                        return
                    end
                    local lower = parent.Name:lower()
                    if lower:find("scp") or lower:find("zombie") then
                        AREXANS_EnsureWorldEntry("SCPZombie", parent)
                        return
                    end
                end
    ...
    local function AREXANS_UnregisterWorldDescendant(obj)
        if not obj then return end
        local validCats = { Generator = true, Hook = true, Gate = true, Window = true, Palletwrong = true }

        if obj:IsA("Model") then
            if validCats[obj.Name] then
                AREXANS_RemoveWorldEntry(obj.Name, obj)
                return
            end
            local lower = obj.Name:lower()
            if lower:find("scp") or lower:find("zombie") then
                AREXANS_RemoveWorldEntry("SCPZombie", obj)
            end
            return
        end

        if obj:IsA("BasePart") then
            for cat, models in pairs(AREXANS_WorldReg) do
                for model, entry in pairs(models) do
                    if entry.part == obj then
                        AREXANS_RemoveWorldEntry(cat, model)
                    end
                end
            end
        end
    ...
    local function AREXANS_AttachESPRoot(root)
        if not root or AREXANS_MapAdd[root] then return end
        AREXANS_MapAdd[root] = root.DescendantAdded:Connect(AREXANS_RegisterWorldDescendant)
        AREXANS_MapRem[root] = root.DescendantRemoving:Connect(AREXANS_UnregisterWorldDescendant)
        for _, descendant in ipairs(root:GetDescendants()) do
            AREXANS_RegisterWorldDescendant(descendant)
        end
    end

    local function AREXANS_RefreshESPRoots()
        for _, conn in pairs(AREXANS_MapAdd) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        for _, conn in pairs(AREXANS_MapRem) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        AREXANS_MapAdd, AREXANS_MapRem = {}, {}

        for cat, models in pairs(AREXANS_WorldReg) do
            for model in pairs(models) do
                AREXANS_ClearWorldVisual(cat, model)
            end
            AREXANS_WorldReg[cat] = {}
        end

        local map = Workspace:FindFirstChild("Map")
        local map1 = Workspace:FindFirstChild("Map1")
        if map then AREXANS_AttachESPRoot(map) end
        if map1 then AREXANS_AttachESPRoot(map1) end
    end

    local function AREXANS_LabelForPallet(model)
        local state = AREXANS_PalletState[model] or "UP"
    ...
    local function AREXANS_LabelForWindow(model)
        local state = AREXANS_WindowState[model] or "READY"
        if state == "BUSY" then return "Window (busy)" end
        return "Window"
    end

    local function AREXANS_AnyWorldEnabled()
        return AREXANS_ESPState.WorldMasterESP and (
            AREXANS_ESPState.GeneratorESP or
            AREXANS_ESPState.HookESP or
            AREXANS_ESPState.GateESP or
            AREXANS_ESPState.WindowESP or
            AREXANS_ESPState.PalletESP or
            AREXANS_ESPState.SCPZombieESP
        )
    end

    local function AREXANS_WorldCategoryData(cat)
        if cat == "Generator" then return AREXANS_ESPState.GeneratorESP, AREXANS_ESPState.GeneratorColor end
        if cat == "Hook" then return AREXANS_ESPState.HookESP, AREXANS_ESPState.HookColor end
        if cat == "Gate" then return AREXANS_ESPState.GateESP, AREXANS_ESPState.GateColor end
        if cat == "Window" then return AREXANS_ESPState.WindowESP, AREXANS_ESPState.WindowColor end
        if cat == "Palletwrong" then return AREXANS_ESPState.PalletESP, AREXANS_ESPState.PalletColor end
        if cat == "SCPZombie" then return AREXANS_ESPState.SCPZombieESP, AREXANS_ESPState.SCPZombieColor end
        return false, Color3.new(1, 1, 1)
    end

    local function AREXANS_UpdateWorldTag(cat, model, part, color)
        local key = AREXANS_WorldKey(cat, model)
        local tagName = key .. "_Tag"
        local folder = AREXANS_GetESPFolder()
        AREXANS_ClearPrefix(tagName, tagName)

        if not AREXANS_ValidPart(part) then
            AREXANS_DestroyChild(tagName)
            return
        end
        ...
        local root = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        local distanceText = ""
        if AREXANS_ESPState.WorldDistanceESP and root then
            distanceText = "[" .. tostring(math.floor((root.Position - part.Position).Magnitude)) .. "m]"
        end

        local nameText = ""
        local labelColor = color
        if AREXANS_ESPState.WorldNametags then
            if cat == "Generator" then
                local txt, genColor = AREXANS_GeneratorLabel(model)
                nameText = txt
                labelColor = genColor
            elseif cat == "Palletwrong" then
                nameText = AREXANS_LabelForPallet(model)
            elseif cat == "Window" then
                nameText = AREXANS_LabelForWindow(model)
            elseif cat == "SCPZombie" then
                nameText = model.Name
            else
                nameText = cat
    ...
    local function AREXANS_ClearAllWorldESP()
        for cat, models in pairs(AREXANS_WorldReg) do
            for model in pairs(models) do
                AREXANS_ClearWorldVisual(cat, model)
            end
    ...
    local function AREXANS_StartWorldLoop()
        if AREXANS_WorldLoopThread then return end
        AREXANS_WorldLoopThread = task.spawn(function()
            while not AREXANS_Dead and AREXANS_AnyWorldEnabled() do
                for cat, models in pairs(AREXANS_WorldReg) do
                    local enabled, color = AREXANS_WorldCategoryData(cat)
                    if enabled and AREXANS_ESPState.WorldMasterESP then
                        local n = 0
                        for model, entry in pairs(models) do
                            if cat == "Palletwrong" and AREXANS_IsPalletGone(model) then
                                AREXANS_RemoveWorldEntry(cat, model)
                            elseif model and AREXANS_Alive(model) then
                                local part = entry.part
                                if not AREXANS_ValidPart(part) or (model:IsA("Model") and not part:IsDescendantOf(model)) then
                                    entry.part = AREXANS_PickWorldPart(model, cat)
                                    part = entry.part
                                end

                                if AREXANS_ValidPart(part) then
                                    local key = AREXANS_WorldKey(cat, model)
                                    AREXANS_EnsureHighlight(key .. "_HL", model, color, false)
                                    AREXANS_UpdateWorldTag(cat, model, part, color)
                                else
                                    AREXANS_RemoveWorldEntry(cat, model)
                                end
                            else
            ...
            AREXANS_WorldLoopThread = nil
        end)
    end

    local function AREXANS_Selected(selected, name)
    ...
    getgenv().AREXANS_AddVisualESPControls = function(VisualTabRef)
        if not VisualTabRef or AREXANS_ControlsAdded then return end
        AREXANS_ControlsAdded = true
        ...
        settingsSection:AddSlider({
            ...
            Flag = "AREXANS ESP Fill Transparency",
            Min = 0,
            Max = 1,
            Default = AREXANS_ESPState.ESPFillTransparency,
            Increment = 0.01,
            Callback = function(value)
                AREXANS_ESPState.ESPFillTransparency = value
                AREXANS_RefreshAllPlayers()
            end,
        })
        ...
        settingsSection:AddSlider({
            ...
            Flag = "AREXANS ESP Outline Transparency",
            Min = 0,
            Max = 1,
            Default = AREXANS_ESPState.ESPOutlineTransparency,
            Increment = 0.01,
            Callback = function(value)
                AREXANS_ESPState.ESPOutlineTransparency = value
                AREXANS_RefreshAllPlayers()
            end,
        })
        ...
        settingsSection:AddSlider({
            Name = "ESP Text Size",
            Flag = "AREXANS ESP Text Size",
            Min = 8,
            Max = 22,
            Default = AREXANS_ESPState.ESPTextSize,
            Increment = 1,
            Callback = function(value)
                AREXANS_ESPState.ESPTextSize = value
                AREXANS_RefreshAllPlayers()
            end,
        })
        ...
        playerSection:AddToggle({
            ...
            Flag = "AREXANS Enable Player ESP",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.PlayerMasterESP = state
                if state then
                    AREXANS_StartPlayerLoop()
                    AREXANS_RefreshAllPlayers()
                else
                    AREXANS_ClearAllPlayerESP()
                end
            end,
        })
        ...
        playerSection:AddDropdown({
            ...
            Flag = "AREXANS Select Player ESP",
            Values = { "Survivor ESP", "Killer ESP", "Spectator ESP", "Survivor Items ESP" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
                AREXANS_ESPState.SurvivorESP = AREXANS_Selected(selected, "Survivor ESP")
                AREXANS_ESPState.KillerESP = AREXANS_Selected(selected, "Killer ESP")
                AREXANS_ESPState.SpectatorESP = AREXANS_Selected(selected, "Spectator ESP")
                AREXANS_ESPState.SurvivorItemsESP = AREXANS_Selected(selected, "Survivor Items ESP")

                if AREXANS_ESPState.PlayerMasterESP then
                    AREXANS_StartPlayerLoop()
                    AREXANS_RefreshAllPlayers()
                else
                    AREXANS_ClearAllPlayerESP()
        ...
        playerSection:AddToggle({
            ...
            Flag = "AREXANS Player Nametags",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.Nametags = state
                if AREXANS_ESPState.PlayerMasterESP then
                    AREXANS_StartPlayerLoop()
                    AREXANS_RefreshAllPlayers()
                else
        ...
        playerSection:AddToggle({
            Name = "Killer Stunned ESP (Pistol)",
            Flag = "AREXANS Killer Stunned ESP",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.KillerStunnedESP = state
                AREXANS_RefreshAllPlayers()
            end,
        })
        ...
        playerSection:AddToggle({
            ...
            Flag = "AREXANS Player Distance ESP",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.DistanceESP = state
                if AREXANS_ESPState.PlayerMasterESP then
                    AREXANS_StartPlayerLoop()
                    AREXANS_RefreshAllPlayers()
                else
        ...
        playerSection:AddColorPicker({ Name = "Survivor Color", Flag = "AREXANS Survivor Color", Default = AREXANS_ESPState.SurvivorColor, Callback = function(color) AREXANS_ESPState.SurvivorColor = color; AREXANS_RefreshAllPlayers() end })
        playerSection:AddColorPicker({ Name = "Killer Color", Flag = "AREXANS Killer Color", Default = AREXANS_ESPState.KillerColor, Callback = function(color) AREXANS_ESPState.KillerColor = color; AREXANS_RefreshAllPlayers() end })
        playerSection:AddColorPicker({ Name = "Spectator Color", Flag = "AREXANS Spectator Color", Default = AREXANS_ESPState.SpectatorColor, Callback = function(color) AREXANS_ESPState.SpectatorColor = color; AREXANS_RefreshAllPlayers() end })
        ...
        worldSection:AddToggle({
            ...
            Flag = "AREXANS Enable World ESP",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.WorldMasterESP = state
                if state then
                    AREXANS_RefreshESPRoots()
                    if AREXANS_AnyWorldEnabled() then AREXANS_StartWorldLoop() end
                else
                    AREXANS_ClearAllWorldESP()
                end
            end,
        })
        ...
        worldSection:AddDropdown({
            ...
            Flag = "AREXANS Select World Objects",
            Values = { "Generators", "Hooks", "Gates", "Windows", "Pallets", "SCP / Zombie" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
                AREXANS_ESPState.GeneratorESP = AREXANS_Selected(selected, "Generators")
                AREXANS_ESPState.HookESP = AREXANS_Selected(selected, "Hooks")
                AREXANS_ESPState.GateESP = AREXANS_Selected(selected, "Gates")
                AREXANS_ESPState.WindowESP = AREXANS_Selected(selected, "Windows")
                AREXANS_ESPState.PalletESP = AREXANS_Selected(selected, "Pallets")
                AREXANS_ESPState.SCPZombieESP = AREXANS_Selected(selected, "SCP / Zombie")

                if AREXANS_ESPState.WorldMasterESP and AREXANS_AnyWorldEnabled() then
                    AREXANS_RefreshESPRoots()
                    AREXANS_StartWorldLoop()
                else
                    AREXANS_ClearAllWorldESP()
                end
            end,
        ...
        worldSection:AddToggle({
            Name = "World Nametags",
            Flag = "AREXANS World Nametags",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.WorldNametags = state
                if AREXANS_ESPState.WorldMasterESP and AREXANS_AnyWorldEnabled() then AREXANS_StartWorldLoop() else AREXANS_ClearAllWorldESP() end
            end,
        })

        worldSection:AddToggle({
            Name = "World Distance ESP",
            Flag = "AREXANS World Distance ESP",
            Default = false,
            Callback = function(state)
                AREXANS_ESPState.WorldDistanceESP = state
                if AREXANS_ESPState.WorldMasterESP and AREXANS_AnyWorldEnabled() then AREXANS_StartWorldLoop() else AREXANS_ClearAllWorldESP() end
            end,
        })

        pcall(function() worldSection:AddDivider({ Text = "Colors" }) end)
        worldSection:AddColorPicker({ Name = "Generator Color", Flag = "AREXANS Generator Color", Default = AREXANS_ESPState.GeneratorColor, Callback = function(color) AREXANS_ESPState.GeneratorColor = color end })
        worldSection:AddColorPicker({ Name = "Hook Color", Flag = "AREXANS Hook Color", Default = AREXANS_ESPState.HookColor, Callback = function(color) AREXANS_ESPState.HookColor = color end })
        worldSection:AddColorPicker({ Name = "Gate Color", Flag = "AREXANS Gate Color", Default = AREXANS_ESPState.GateColor, Callback = function(color) AREXANS_ESPState.GateColor = color end })
        worldSection:AddColorPicker({ Name = "Window Color", Flag = "AREXANS Window Color", Default = AREXANS_ESPState.WindowColor, Callback = function(color) AREXANS_ESPState.WindowColor = color end })
        worldSection:AddColorPicker({ Name = "Pallet Color", Flag = "AREXANS Pallet Color", Default = AREXANS_ESPState.PalletColor, Callback = function(color) AREXANS_ESPState.PalletColor = color end })
        worldSection:AddColorPicker({ Name = "SCP / Zombie Color", Flag = "AREXANS SCP Zombie Color", Default = AREXANS_ESPState.SCPZombieColor, Callback = function(color) AREXANS_ESPState.SCPZombieColor = color end })
    end

    for _, player in ipairs(Players:GetPlayers()) do
        AREXANS_WatchPlayer(player)
    end

    table.insert(AREXANS_Connections, Players.PlayerAdded:Connect(AREXANS_WatchPlayer))
    table.insert(AREXANS_Connections, Players.PlayerRemoving:Connect(AREXANS_UnwatchPlayer))
    table.insert(AREXANS_Connections, Workspace.ChildAdded:Connect(function(child)
        if child.Name == "Map" or child.Name == "Map1" then
            AREXANS_AttachESPRoot(child)
            if AREXANS_ESPState.WorldMasterESP and AREXANS_AnyWorldEnabled() then AREXANS_StartWorldLoop() end
        end
    end))
    table.insert(AREXANS_Connections, Workspace.ChildRemoved:Connect(function(child)
        if child.Name == "Map" or child.Name == "Map1" then
            AREXANS_RefreshESPRoots()
        end
    end))
    ...
    getgenv().AREXANS_VD_VisualESP_Cleanup = function()
        AREXANS_Dead = true
        AREXANS_ClearAllPlayerESP()
        AREXANS_ClearAllWorldESP()

        for _, conn in ipairs(AREXANS_Connections) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        for _, conns in pairs(AREXANS_PlayerConns) do
        ...
        if AREXANS_ESPFolder and AREXANS_ESPFolder.Parent then
            AREXANS_ESPFolder:Destroy()
        end
    end

    AREXANS_SafeNotify("Visual ESP", "Highlight ESP V2 loaded. Anti double nametag aktif.", 3)
end
...
task.spawn(function()
    while not VD.Destroyed do
        if VD.Fullbright then
            local weatherTheme = VD.VIS_WeatherTheme and AREXANS_WeatherPresets[VD.VIS_WeatherTheme]
            ...
            if VD.VIS_WeatherTheme and VD.VIS_WeatherTheme ~= "Default" and AREXANS_WeatherPresets[VD.VIS_WeatherTheme] then
                local theme = AREXANS_WeatherPresets[VD.VIS_WeatherTheme]
                if theme.Lighting then
                    for k, v in pairs(theme.Lighting) do
                        pcall(function() Lighting[k] = v end)
                    end
...
local function AREXANS_RegisterNoclipPart(obj)
    if not obj or not obj:IsA("BasePart") then return end
    noclipParts[obj] = true
    if originalCanCollide[obj] == nil then
        originalCanCollide[obj] = obj.CanCollide
    end
end

local function AREXANS_BuildNoclipCache(char)
    table.clear(noclipParts)
    if noclipDescConn then pcall(function() noclipDescConn:Disconnect() end) end
    noclipDescConn = nil
    if not char then return end
    for _, obj in ipairs(char:GetDescendants()) do
        AREXANS_RegisterNoclipPart(obj)
    end
    noclipDescConn = char.DescendantAdded:Connect(AREXANS_RegisterNoclipPart)
end

AREXANS_BuildNoclipCache(LocalPlayer.Character)
LocalPlayer.CharacterAdded:Connect(function(char)
    task.defer(function() AREXANS_BuildNoclipCache(char) end)
end)
...
local function AREXANS_ToFGetEvent()
...
local function AREXANS_ToFGetGunObject()
    local char = LocalPlayer.Character
...
local function AREXANS_ToFIsTargetVisible(originPos, targetPos, targetCharacter)
    ...
    if AREXANS_ToFState.LaserBeam then table.insert(excludeList, AREXANS_ToFState.LaserBeam) end
...
local function AREXANS_ToFGetSCPs()
    if tick() - AREXANS_ToFState.SCPCacheTimer < 0.5 then
        return AREXANS_ToFState.SCPCache
    end
    ...
    AREXANS_ToFState.SCPCache = newTargets
    AREXANS_ToFState.SCPCacheTimer = tick()
    return AREXANS_ToFState.SCPCache
end

local function AREXANS_ToFGetTargetPosition()
    local gunObj = AREXANS_ToFGetGunObject()
    local char = LocalPlayer.Character
    ...
    local function predictTarget(torso, targetCharacter)
        ...
        if VD.TOF_WallCheck and not AREXANS_ToFIsTargetVisible(originPos, targetPos, targetCharacter) then
    ...
    elseif targetMode == "Zombie" then
        ...
        for _, root in ipairs(AREXANS_ToFGetSCPs()) do
...
local function AREXANS_ToFUpdateLaser(originPos, targetPos)
    if not AREXANS_ToFState.LaserBeam then
        ...
        AREXANS_ToFState.LaserBeam = laser
    end

    local dist = (targetPos - originPos).Magnitude
    AREXANS_ToFState.LaserBeam.Size = Vector3.new(0.05, 0.05, dist)
    AREXANS_ToFState.LaserBeam.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
    AREXANS_ToFState.LaserBeam.Transparency = 0
end

local function AREXANS_ToFClearLaser()
    if AREXANS_ToFState.LaserBeam then
        pcall(function() AREXANS_ToFState.LaserBeam:Destroy() end)
        AREXANS_ToFState.LaserBeam = nil
    end
end
...
local function AREXANS_ToFGetMobileShootButton()
...
local function AREXANS_ToFIsTouchOnShootButton(input)
    local shootButton = AREXANS_ToFGetMobileShootButton()
...
local function AREXANS_ToFDoShoot()
    ...
    local targetDirection, gunObject, originPos, targetPos = AREXANS_ToFGetTargetPosition()
    if not (targetDirection and gunObject and targetPos and originPos) then return end

    local tofEvent = AREXANS_ToFGetEvent()
...
local AREXANS_ToFModeButtons = {}
local function AREXANS_ToFRefreshTargetButtons()
    local modes = {
    ...
    for modeName, btn in pairs(AREXANS_ToFModeButtons) do
...
local function AREXANS_ToFSetTargetMode(modeName, notify)
    if modeName ~= "Killer" and modeName ~= "Survivors" and modeName ~= "Zombie" then return end
    VD.TOF_TargetMode = modeName
    AREXANS_ToFRefreshTargetButtons()
    if notify then VD_Notify("Target Mode", modeName, 1) end
end

local function AREXANS_ToFCreateTargetSelectorUI()
    local parent = GetSafeGuiParent()
    if not parent then return end
    if AREXANS_ToFState.TargetGui and AREXANS_ToFState.TargetGui.Parent then return end
    ...
    frame.Position = AREXANS_ToFState.SavedUIPos
    ...
    for i, mode in ipairs(modes) do
        ...
        btn.MouseButton1Click:Connect(function()
            AREXANS_ToFSetTargetMode(mode.Internal, false)
        end)
        ...
        AREXANS_ToFModeButtons[mode.Internal] = btn
    end
    AREXANS_ToFRefreshTargetButtons()
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            ...
            AREXANS_ToFState.SavedUIPos = newPos
        end
    end)

    AREXANS_ToFState.TargetGui = gui
end

local function AREXANS_ToFDestroyTargetSelectorUI()
    if AREXANS_ToFState.TargetGui then
        pcall(function() AREXANS_ToFState.TargetGui:Destroy() end)
        AREXANS_ToFState.TargetGui = nil
    end
    AREXANS_ToFModeButtons = {}
end

local function AREXANS_ToFStartConnection()
    if AREXANS_ToFState.Connection then return end
    AREXANS_ToFState.Connection = RunService.Heartbeat:Connect(function()
        if not VD.TOF_SilentAim or not AREXANS_ToFState.IsAiming then
            if AREXANS_ToFState.LaserBeam then AREXANS_ToFState.LaserBeam.Transparency = 1 end
            return
        end

        local _, _, originPos, targetPos = AREXANS_ToFGetTargetPosition()
        if originPos and targetPos then
            ...
            if VD.TOF_Laser then
                AREXANS_ToFUpdateLaser(originPos, targetPos)
            elseif AREXANS_ToFState.LaserBeam then
                AREXANS_ToFState.LaserBeam.Transparency = 1
            end
        elseif AREXANS_ToFState.LaserBeam then
            AREXANS_ToFState.LaserBeam.Transparency = 1
        end
...
local function AREXANS_ToFStopConnection()
    if AREXANS_ToFState.Connection then
        pcall(function() AREXANS_ToFState.Connection:Disconnect() end)
        AREXANS_ToFState.Connection = nil
    end
    AREXANS_ToFState.IsAiming = false
    AREXANS_ToFClearLaser()
end

local function AREXANS_ToFDisconnectInputs()
    if AREXANS_ToFState.InputBegan then pcall(function() AREXANS_ToFState.InputBegan:Disconnect() end) end
    if AREXANS_ToFState.InputEnded then pcall(function() AREXANS_ToFState.InputEnded:Disconnect() end) end
    AREXANS_ToFState.InputBegan = nil
    AREXANS_ToFState.InputEnded = nil
end

local AREXANS_SetToFSilentAim

local function AREXANS_ToFEnsureInputs()
    if not AREXANS_ToFState.InputBegan then
        AREXANS_ToFState.InputBegan = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed then return end

            local keyCode = AREXANS_ToFKeyCodes[VD.TOF_Key or "None"]
            if keyCode and input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == keyCode then
                AREXANS_SetToFSilentAim(not VD.TOF_SilentAim)
                return
            end

            if not VD.TOF_SilentAim then return end
            if input.UserInputType == Enum.UserInputType.MouseButton1
            or (input.UserInputType == Enum.UserInputType.Touch and AREXANS_ToFIsTouchOnShootButton(input)) then
                AREXANS_ToFState.IsAiming = true
                if input.UserInputType == Enum.UserInputType.Touch then
                    AREXANS_ToFState.TouchInput = input
                end
                AREXANS_ToFDoShoot()
                return
            end
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard then
                if input.KeyCode == Enum.KeyCode.K then
                    AREXANS_ToFSetTargetMode("Killer", true)
                elseif input.KeyCode == Enum.KeyCode.J then
                    AREXANS_ToFSetTargetMode("Survivors", true)
                elseif input.KeyCode == Enum.KeyCode.L then
                    AREXANS_ToFSetTargetMode("Zombie", true)
                end
            end
        end)
    end
    if not AREXANS_ToFState.InputEnded then
        AREXANS_ToFState.InputEnded = UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
            or (input.UserInputType == Enum.UserInputType.Touch and input == AREXANS_ToFState.TouchInput) then
                AREXANS_ToFState.IsAiming = false
                if input == AREXANS_ToFState.TouchInput then AREXANS_ToFState.TouchInput = nil end
                if AREXANS_ToFState.LaserBeam then AREXANS_ToFState.LaserBeam.Transparency = 1 end
            end
        end)
    end
...
AREXANS_SetToFSilentAim = function(enabled)
    VD.TOF_SilentAim = enabled and true or false
    AREXANS_ToFEnsureInputs()
    if VD.TOF_SilentAim then
        -- Target Mode GUI tidak lagi ditampilkan di layar saat Silent Aim pistol aktif.
        -- Pengaturan target mode tetap tersedia melalui setting/keybind yang sudah ada.
        AREXANS_ToFDestroyTargetSelectorUI()
        AREXANS_ToFStartConnection()
    else
        AREXANS_ToFDestroyTargetSelectorUI()
        AREXANS_ToFStopConnection()
    end
end

AREXANS_ToFEnsureInputs()
getgenv().AREXANS_SetToFSilentAim = AREXANS_SetToFSilentAim
getgenv().AREXANS_ToFClearLaser = AREXANS_ToFClearLaser
getgenv().AREXANS_ToFSetTargetMode = AREXANS_ToFSetTargetMode
end)();
...
-- =====================================================
(function()
local AREXANS_HideSurvivorIconState = {
    Connection = nil,
    Originals = {},
}

local AREXANS_HideSurvivorIconImage = "rbxassetid://111967366195505"
local AREXANS_HideSurvivorIconText = "NxH"

local function AREXANS_GetSurvivorSlots()
    local slots = {}
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
...
local function AREXANS_ApplyHideSurvivorIcon()
    for _, slot in ipairs(AREXANS_GetSurvivorSlots()) do
        local imageLabel = slot.ImageLabel
        if imageLabel and imageLabel:IsA("ImageLabel") then
            if not AREXANS_HideSurvivorIconState.Originals[imageLabel] then
                AREXANS_HideSurvivorIconState.Originals[imageLabel] = {
            ...
            imageLabel.Image = AREXANS_HideSurvivorIconImage
        ...
        if textLabel and textLabel:IsA("TextLabel") then
            if not AREXANS_HideSurvivorIconState.Originals[textLabel] then
                AREXANS_HideSurvivorIconState.Originals[textLabel] = {
                    Text = textLabel.Text,
            ...
            textLabel.Text = AREXANS_HideSurvivorIconText
...
local function AREXANS_RestoreSurvivorIcons()
    for object, original in pairs(AREXANS_HideSurvivorIconState.Originals) do
    ...
    AREXANS_HideSurvivorIconState.Originals = {}
end

local function AREXANS_SetHideSurvivorIcon(enabled)
    VD.VIS_HideSurvivorIcon = enabled and true or false

    if VD.VIS_HideSurvivorIcon then
        AREXANS_ApplyHideSurvivorIcon()
        if not AREXANS_HideSurvivorIconState.Connection then
            AREXANS_HideSurvivorIconState.Connection = RunService.Heartbeat:Connect(function()
                if VD.VIS_HideSurvivorIcon then
                    AREXANS_ApplyHideSurvivorIcon()
                end
            end)
        ...
        if AREXANS_HideSurvivorIconState.Connection then
            pcall(function() AREXANS_HideSurvivorIconState.Connection:Disconnect() end)
            AREXANS_HideSurvivorIconState.Connection = nil
        end
        AREXANS_RestoreSurvivorIcons()
    end
end

getgenv().AREXANS_SetHideSurvivorIcon = AREXANS_SetHideSurvivorIcon
end)();
...
-- =====================================================
(function()
local AREXANS_HookCounterState = {
    Connection = nil,
}

local function AREXANS_UpdateHookCounter(enabled)
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
            ...
            if frame then
                for i = 1, 5 do
                    ...
                    if imageLabel and textLabel then
                        ...
                        local labelName = "AREXANS_CustomHookCounter"
...
local function AREXANS_SetShowHookCounter(enabled)
    VD.VIS_ShowHookCounter = enabled and true or false

    if VD.VIS_ShowHookCounter then
        AREXANS_UpdateHookCounter(true)
        if not AREXANS_HookCounterState.Connection then
            AREXANS_HookCounterState.Connection = RunService.Heartbeat:Connect(function()
                if VD.VIS_ShowHookCounter then
                    AREXANS_UpdateHookCounter(true)
                end
            end)
        ...
        if AREXANS_HookCounterState.Connection then
            pcall(function() AREXANS_HookCounterState.Connection:Disconnect() end)
            AREXANS_HookCounterState.Connection = nil
        end
        AREXANS_UpdateHookCounter(false)
                ...
                if gui:IsA("ScreenGui") and gui.Name:match("%-mob$") then
                    ...
                    if frame then
                        for i = 1, 5 do
                            local survivorFrame = frame:FindFirstChild("Survivor" .. i)
                            local imageLabel = survivorFrame and survivorFrame:FindFirstChild("ImageLabel")
                            customLabel = imageLabel and imageLabel:FindFirstChild("AREXANS_CustomHookCounter")
                            if customLabel then
...
getgenv().AREXANS_SetShowHookCounter = AREXANS_SetShowHookCounter
end)();
...
local function AREXANS_GetPingValue()
...
local function AREXANS_CreatePingFPSGui()
    local parent = GetSafeGuiParent()
    if not parent then return nil end

    local old = parent:FindFirstChild("AREXANS_PingFPSGui")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "AREXANS_PingFPSGui"
...
local function AREXANS_SetShowPingFPS(enabled)
    VD.VIS_ShowPingFPS = enabled and true or false

    if VD.VIS_ShowPingFPS then
        AREXANS_PingFPSState.Gui = AREXANS_PingFPSState.Gui or AREXANS_CreatePingFPSGui()
        AREXANS_PingFPSState.Frames = 0
        AREXANS_PingFPSState.LastUpdate = tick()

        if not AREXANS_PingFPSState.Connection then
            AREXANS_PingFPSState.Connection = RunService.RenderStepped:Connect(function()
                if not VD.VIS_ShowPingFPS then return end

                AREXANS_PingFPSState.Frames = AREXANS_PingFPSState.Frames + 1
                local now = tick()
                if now - AREXANS_PingFPSState.LastUpdate < 0.5 then return end

                local fps = math.floor(AREXANS_PingFPSState.Frames / (now - AREXANS_PingFPSState.LastUpdate) + 0.5)
                local ping = AREXANS_GetPingValue()
                AREXANS_PingFPSState.Frames = 0
                AREXANS_PingFPSState.LastUpdate = now

                if not (AREXANS_PingFPSState.Gui and AREXANS_PingFPSState.Gui.Parent) then
                    AREXANS_PingFPSState.Gui = AREXANS_CreatePingFPSGui()
                end

                local label = AREXANS_PingFPSState.Gui and AREXANS_PingFPSState.Gui:FindFirstChild("PingFPSLabel", true)
                if label then
        ...
        if AREXANS_PingFPSState.Connection then
            pcall(function() AREXANS_PingFPSState.Connection:Disconnect() end)
            AREXANS_PingFPSState.Connection = nil
        end
        if AREXANS_PingFPSState.Gui then
            pcall(function() AREXANS_PingFPSState.Gui:Destroy() end)
            AREXANS_PingFPSState.Gui = nil
        end
    end
end

getgenv().AREXANS_SetShowPingFPS = AREXANS_SetShowPingFPS
end)();
...
-- =====================================================
(function()
local AREXANS_FlashlightAimState = {
    Connection = nil,
    LaserBeam = nil,
    FlashlightPart = nil,
    Active = false,
...
local function AREXANS_GetFlashlightActivateRemote()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    local items = remotes and remotes:FindFirstChild("Items")
...
local function AREXANS_GetFlashlightTargetPart(char)
    if not char then return nil end
...
local function AREXANS_IsAliveCharacter(char)
...
local function AREXANS_GetFlashlightTarget()
    local localChar = LocalPlayer.Character
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and AREXANS_IsAliveCharacter(player.Character) then
            local isKiller = player.Team and player.Team.Name == "Killer"
            if isKiller then
                local part = AREXANS_GetFlashlightTargetPart(player.Character)
                if part then
...
local function AREXANS_ClearFlashlightLaser()
    if AREXANS_FlashlightAimState.LaserBeam then
        pcall(function() AREXANS_FlashlightAimState.LaserBeam:Destroy() end)
        AREXANS_FlashlightAimState.LaserBeam = nil
    end
end

local function AREXANS_GetFlashlightOrigin(cam)
    local source = AREXANS_FlashlightAimState.FlashlightPart
...
local function AREXANS_UpdateFlashlightLaser(originPos, targetPos)
    if not AREXANS_FlashlightAimState.LaserBeam then
        local laser = Instance.new("Part")
        ...
        laser.Parent = workspace
        AREXANS_FlashlightAimState.LaserBeam = laser
    end

    local dist = (targetPos - originPos).Magnitude
    if dist < 0.1 then return end

    local laser = AREXANS_FlashlightAimState.LaserBeam
    laser.Size = Vector3.new(0.16, 0.16, dist)
    laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
    laser.Transparency = 0
end

local function AREXANS_FlashlightAimStep()
    if false then
        if AREXANS_FlashlightAimState.LaserBeam then
            AREXANS_FlashlightAimState.LaserBeam.Transparency = 1
        end
        return
    end

    if not (VD.FLASH_SilentAim and AREXANS_FlashlightAimState.Active) then
        if AREXANS_FlashlightAimState.LaserBeam then
            AREXANS_FlashlightAimState.LaserBeam.Transparency = 1
        end
        return
    ...
    local cam = workspace.CurrentCamera
    local targetPart = AREXANS_GetFlashlightTarget()
    ...
    local originPos = AREXANS_GetFlashlightOrigin(cam)

    if VD.FLASH_Laser and originPos then
        AREXANS_UpdateFlashlightLaser(originPos, targetPos)
    elseif AREXANS_FlashlightAimState.LaserBeam then
        AREXANS_FlashlightAimState.LaserBeam.Transparency = 1
    end
...
local function AREXANS_StartFlashlightSilentAim()
    getgenv().AREXANS_FlashlightActivateRemote = AREXANS_GetFlashlightActivateRemote()
    if AREXANS_FlashlightAimState.Connection then return end
    AREXANS_FlashlightAimState.Connection = RunService.RenderStepped:Connect(AREXANS_FlashlightAimStep)
end

local function AREXANS_StopFlashlightSilentAim()
    AREXANS_FlashlightAimState.Active = false
    AREXANS_FlashlightAimState.FlashlightPart = nil
    AREXANS_ClearFlashlightLaser()
    if AREXANS_FlashlightAimState.Connection then
        pcall(function() AREXANS_FlashlightAimState.Connection:Disconnect() end)
        AREXANS_FlashlightAimState.Connection = nil
    end
end

local function AREXANS_SetFlashlightSilentAim(enabled)
    if enabled and false then
        VD.FLASH_SilentAim = false
        AREXANS_StopFlashlightSilentAim()
        return
    end

    VD.FLASH_SilentAim = enabled and true or false
    if VD.FLASH_SilentAim then
        AREXANS_StartFlashlightSilentAim()
    else
        AREXANS_StopFlashlightSilentAim()
    end
end

getgenv().AREXANS_SetFlashlightSilentAim = AREXANS_SetFlashlightSilentAim
getgenv().AREXANS_ClearFlashlightLaser = AREXANS_ClearFlashlightLaser
getgenv().AREXANS_SetFlashlightAimActive = function(active, flashlightPart)
    AREXANS_FlashlightAimState.Active = active and true or false
    if AREXANS_FlashlightAimState.Active and flashlightPart then
        AREXANS_FlashlightAimState.FlashlightPart = flashlightPart
    elseif not AREXANS_FlashlightAimState.Active then
        AREXANS_FlashlightAimState.FlashlightPart = nil
    end
    if not AREXANS_FlashlightAimState.Active and AREXANS_FlashlightAimState.LaserBeam then
        AREXANS_FlashlightAimState.LaserBeam.Transparency = 1
    end
end
getgenv().AREXANS_FlashlightActivateRemote = AREXANS_GetFlashlightActivateRemote()
...
VD_ParryRange.Name = "AREXANS_ParryRange"
    ...
    if VeilState.chargingSpear and VeilConfig.Enabled and isSpearMode then
        ...
        if target and target.Part and target.Part.Parent then
            ...
            if VeilConfig.ShowTargetLaser then
                if not getgenv().AREXANS_SpearLaserPart then
                    local laser = Instance.new("Part")
                    ...
                    laser.Transparency = 0
                    laser.Parent = workspace
                    getgenv().AREXANS_SpearLaserPart = laser
                end
                ...
                if originPart then
                    ...
                    local dist = (targetPos - originPos).Magnitude
                    if dist > 0.1 then
                        local laser = getgenv().AREXANS_SpearLaserPart
                        laser.Size = Vector3.new(0.16, 0.16, dist)
                        laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
                ...
                if getgenv().AREXANS_SpearLaserPart then getgenv().AREXANS_SpearLaserPart.Transparency = 1 end
            end
        else
...
local AREXANS_MainInfoPanel = {
    Widgets = {},
    Texts = {},
}

function AREXANS_InfoPlainText(text)
...
function AREXANS_UpdateInfoWidget(widget, text)
...
function AREXANS_SetMainInfoPanelText(key, title, text)
    local value = tostring(title or key) .. "\n" .. AREXANS_InfoPlainText(text)
    AREXANS_MainInfoPanel.Texts[key] = value
    AREXANS_UpdateInfoWidget(AREXANS_MainInfoPanel.Widgets[key], value)
end

function AREXANS_RegisterMainInfoWidget(key, widget)
    AREXANS_MainInfoPanel.Widgets[key] = widget
    if AREXANS_MainInfoPanel.Texts[key] then
        AREXANS_UpdateInfoWidget(widget, AREXANS_MainInfoPanel.Texts[key])
    end
end

function AREXANS_AddMainInfoLine(section, key, title, defaultText)
    local defaultValue = tostring(title) .. "\n" .. tostring(defaultText or "Off")
    AREXANS_MainInfoPanel.Texts[key] = AREXANS_MainInfoPanel.Texts[key] or defaultValue
    ...
    if ok and widget then return AREXANS_RegisterMainInfoWidget(key, widget) end
...
if Window then
    ...
    -- Search akan memilih TAB + SUB-TAB pemilik fitur secara otomatis.
    -- =====================================================
    local AREXANS_SearchFeatureRegistry = {}
    local AREXANS_SearchTabRegistry = {}

    local function AREXANS_NormalizeSearchText(value)
        value = tostring(value or ""):lower()
        value = value:gsub("[%p]$", "")
        return value
    end

    local function AREXANS_SearchMatches(query, keyword)
        query = AREXANS_NormalizeSearchText(query)
        keyword = AREXANS_NormalizeSearchText(keyword)
        if query == "" or keyword == "" then return false end
        return keyword:find(query, 1, true) ~= nil or query:find(keyword, 1, true) ~= nil
    end

    local function AREXANS_RegisterSearchTab(tabRef, name, parentTab)
        if not tabRef or not name then return end
        table.insert(AREXANS_SearchTabRegistry, {
            Name = tostring(name),
            Tab = tabRef,
            ParentTab = parentTab,
            Keywords = { AREXANS_NormalizeSearchText(name) },
        })
    end

    local function AREXANS_RegisterSearchFeature(tabRef, name, parentTab)
        if not tabRef or not name then return end
        local text = tostring(name)
        table.insert(AREXANS_SearchFeatureRegistry, {
            Name = text,
            Tab = tabRef,
            ParentTab = parentTab,
            Keywords = { AREXANS_NormalizeSearchText(text) },
        })
    end

    local function AREXANS_SelectSearchTarget(query)
        query = AREXANS_NormalizeSearchText(query)
        if query == "" then return false end

        -- 1. Prioritas nama fitur/kontrol.
        for _, entry in ipairs(AREXANS_SearchFeatureRegistry) do
            for _, keyword in ipairs(entry.Keywords) do
                if AREXANS_SearchMatches(query, keyword) then
                    pcall(function()
                        -- ParentTab = tab utama (Visual/Main/Aim/Mapping/Player)
                        if entry.ParentTab and type(entry.ParentTab.Select) == "function" then
                            entry.ParentTab:Select()
                        end
                        -- Tab = sub-tab pemilik fitur (Survivor/Aimbot/etc).
        ...
        -- 2. Fallback nama sub-tab.
        for _, entry in ipairs(AREXANS_SearchTabRegistry) do
            if AREXANS_SearchMatches(query, entry.Name) then
                pcall(function()
                    if entry.ParentTab and type(entry.ParentTab.Select) == "function" then
                        entry.ParentTab:Select()
                    end
        ...
        -- 3. Fallback langsung ke registry internal ModernV2.
        -- Ini penting untuk kontrol yang dibuat langsung ke Window/UserSettings
        -- dan tidak melewati adapter di atas.
        if ModernV2 and ModernV2.NameRegisitry then
            for _, item in pairs(ModernV2.NameRegisitry) do
                if AREXANS_SearchMatches(query, item.Idx) then
                    pcall(function()
                        local root = item.Root

                        -- Kontrol di User Settings.
                        local settingsRoot = Window.UserSettings and Window.UserSettings.Root
                        local parent = root
                        while parent do
                            if settingsRoot and parent == settingsRoot then
                        ...
                        -- Kontrol di tab utama / sub-tab.
                        for _, tabEntry in ipairs(AREXANS_SearchTabRegistry) do
                            local tabRoot = tabEntry.Tab and tabEntry.Tab.Root
                            local cur = root
                            while cur do
                                if tabRoot and cur == tabRoot then
                                    if tabEntry.ParentTab and type(tabEntry.ParentTab.Select) == "function" then
                                        tabEntry.ParentTab:Select()
                                    end
                                    if tabEntry.Tab and type(tabEntry.Tab.Select) == "function" then
                                        tabEntry.Tab:Select()
                                    end
                                    return
                                end
                        ...
                        -- Tab utama terakhir.
                        for _, mainTab in pairs(Tabs) do
                            local tabRoot = mainTab and mainTab.Root
                            local cur = root
                            while cur do
                                if tabRoot and cur == tabRoot then
                                    if type(mainTab.Select) == "function" then
                                        mainTab:Select()
                                    end
                                    return
                                end
    ...
    local function AREXANS_RevealSearchMatch(query)
        query = AREXANS_NormalizeSearchText(query)
        if query == "" then return end

        -- ModernV2 sendiri menyimpan semua kontrol pada NameRegisitry.
        -- Setelah tab/sub-tab dipilih, RevealQueryItem akan membawa UI
        -- langsung ke kontrol yang cocok jika API tersebut tersedia.
        task.defer(function()
            pcall(function()
                if ModernV2 and ModernV2.NameRegisitry and ModernV2.RevealQueryItem then
                    for _, item in pairs(ModernV2.NameRegisitry) do
                        local idx = AREXANS_NormalizeSearchText(item.Idx)
                        if idx ~= "" and AREXANS_SearchMatches(query, idx) then
                            ModernV2:RevealQueryItem(item)
                            break
                        end
                    end
                end
    ...
    local function AREXANS_BindGlobalSearch()
        local bound = setmetatable({}, { __mode = "k" })

        local function bindRoot(root)
            if not root or not root.GetDescendants then return end
            for _, obj in ipairs(root:GetDescendants()) do
                if obj:IsA("TextBox") and not bound[obj] then
                    local placeholder = tostring(obj.PlaceholderText or ""):lower()
                    local name = tostring(obj.Name or ""):lower()
                    if placeholder:find("search", 1, true) or name:find("search", 1, true) then
                        bound[obj] = true
                        obj:GetPropertyChangedSignal("Text"):Connect(function()
                            local text = tostring(obj.Text or "")
                            if text ~= "" then
                                -- Beri waktu singkat agar registry/framework selesai
                                -- memproses input, lalu pindah ke lokasi fitur.
                                task.defer(function()
                                    if AREXANS_SelectSearchTarget(text) then
                                        AREXANS_RevealSearchMatch(text)
                                    end
                                end)
                            end
                        end)
        ...
        local root = ModernV2 and ModernV2.ScreenGui
        if root then
            bindRoot(root)
            root.DescendantAdded:Connect(function(obj)
                if obj:IsA("TextBox") then
                    task.defer(function() bindRoot(root) end)
                end
            end)
        end
    end

    local function makeModernAdapter(section, searchTabRef, searchParentTab)
        local adapter = {}
        adapter.__AREXANS_SearchTab = searchTabRef or section
        adapter.__AREXANS_SearchParentTab = searchParentTab
        ...
        setmetatable(adapter, {
            __index = function(t, k)
                ...
                if k == "AddSlider" then
                    return function(self, cfg)
                        if cfg and cfg.Name then
                            AREXANS_RegisterSearchFeature(adapter.__AREXANS_SearchTab, cfg.Name, adapter.__AREXANS_SearchParentTab)
                            local modernCfg = {
                                Name = cfg.Name,
                            ...
                            if isFloat then
                                modernCfg.Rounding = (modernCfg.Increment <= 0.01) and 2 or 1
                            else
                ...
                if type(section[k]) == "function" then
                    return function(self, ...)
                        local args = {...}
                        local cfg = args[1]
                        if type(cfg) == "table" and cfg.Name and tostring(k):sub(1, 3) == "Add" and k ~= "AddSection" then
                            AREXANS_RegisterSearchFeature(
                                adapter.__AREXANS_SearchTab,
                                cfg.Name,
                                adapter.__AREXANS_SearchParentTab
                            )
                        end
            ...
            __index = function(t, k)
                if k == "AddSection" then
                    return function(self, cfg)
                        ...
                        return makeModernAdapter(tab, tab, nil)
                    end
                end
                return tab[k]
            end
    ...
    local function addCenterFeatureTabbox(tab, name, entries)
        ...
        for _, entry in ipairs(entries) do
            local subTab = tabbox:AddTab({
                Name = entry.Name,
                Icon = entry.Icon,
            })
            created[entry.Key] = makeModernAdapter(subTab, subTab, tab)
            AREXANS_RegisterSearchTab(subTab, entry.Name, tab)
        end

        return created
    end

    -- Register main tabs as fallback targets too.
    for _, tab in pairs(Tabs) do
        AREXANS_RegisterSearchTab(tab, tab.Name or "", nil)
    end
    ...
    -- Bind setelah SearchBox dibuat oleh ModernV2.
    task.defer(AREXANS_BindGlobalSearch)
end
...
do -- ESP Tab
    ...
    pcall(function()
        if getgenv().AREXANS_AddVisualESPControls then
            getgenv().AREXANS_AddVisualESPControls(VisualTab)
        end
    end)
...
do -- Aim Tab
    ...
    flaskSection:AddToggle({ Default = false, Name = "Flask Laser (Cure)", Locked = false, TextLocked = "Premium Required", Flag = "Flask Laser (Cure)", Callback = function(v)
        ...
        if v then
            pcall(AREXANS_StartCureFlaskLaser)
        else
            if getgenv().AREXANS_CureFlaskLaserThread then
                getgenv().AREXANS_CureFlaskLaserThread:Disconnect()
                getgenv().AREXANS_CureFlaskLaserThread = nil
            end
            if getgenv().AREXANS_CureFlaskLaserPart then
                pcall(function() getgenv().AREXANS_CureFlaskLaserPart:Destroy() end)
                getgenv().AREXANS_CureFlaskLaserPart = nil
            end
        end
    end })
    ...
    tofSection:AddToggle({
        ...
        Callback = function(v)
            ...
            if getgenv().AREXANS_SetToFSilentAim then
                getgenv().AREXANS_SetToFSilentAim(v)
            end
        end
    })
    tofSection:AddToggle({
        ...
        Callback = function(v)
            ...
            VD.TOF_Laser = v
            if not v and getgenv().AREXANS_ToFClearLaser then
                getgenv().AREXANS_ToFClearLaser()
            end
        end
    })
    tofSection:AddToggle({
    ...
    tofSection:AddDropdown({
        ...
        Callback = function(v)
            if type(v) == "table" then v = v[1] end
            if getgenv().AREXANS_ToFSetTargetMode then
                getgenv().AREXANS_ToFSetTargetMode(v or "Killer", false)
            else
    ...
    flashlightSection:AddToggle({
        ...
        Callback = function(v)
            ...
            if getgenv().AREXANS_SetFlashlightSilentAim then
                getgenv().AREXANS_SetFlashlightSilentAim(v)
            else
    ...
    flashlightSection:AddToggle({
        ...
        Callback = function(v)
            ...
            VD.FLASH_Laser = v
            if not v and getgenv().AREXANS_ClearFlashlightLaser then
                getgenv().AREXANS_ClearFlashlightLaser()
            end
        end
    })
...
do -- FOV Tab
    ...
    AREXANS_AddMainInfoLine(infoPanelSection, "XanstKiller", "Xanst Killer Display", "Off")
    AREXANS_AddMainInfoLine(infoPanelSection, "KillerPerks", "Spectate Killer Perks", "Off")
    AREXANS_AddMainInfoLine(infoPanelSection, "PredictMap", "Predict Map", "Off")

    visualSection:AddToggle({ Default = false, Name = "Xanst Killer Display", Flag = "Xanst Killer Display", Callback = function(v) 
        VD.VIS_XanstKiller = v 
        if v then
            StartXanstKiller()
        else
            StopXanstKiller()
    ...
    visualSection:AddToggle({ Default = false, Name = "Hide Survivor Icon", Flag = "Hide Survivor Icon", Callback = function(v)
        if getgenv().AREXANS_SetHideSurvivorIcon then
            getgenv().AREXANS_SetHideSurvivorIcon(v)
        else
            VD.VIS_HideSurvivorIcon = v
        end
    end })
    visualSection:AddToggle({ Default = false, Name = "Show Ping & FPS", Flag = "Show Ping & FPS", Callback = function(v)
        if getgenv().AREXANS_SetShowPingFPS then
            getgenv().AREXANS_SetShowPingFPS(v)
        else
    ...
    visualSection:AddToggle({ Default = false, Name = "Show Hook Counter", Flag = "Show Hook Counter", Callback = function(v)
        if getgenv().AREXANS_SetShowHookCounter then
            getgenv().AREXANS_SetShowHookCounter(v)
        else
...
getgenv().AREXANS_FakeParryTrack = nil
...
function setupFakeParryBtn()
    ...
    -- Mini button + logo di sisi kanan layar.
    FakeParryData.Button.Size = UDim2.new(0, 44, 0, 44)
    FakeParryData.Button.Position = UDim2.new(1, -70, 0.75, 0)
    ...
    -- Logo Fake Parry, tanpa tulisan besar.
    local logo = Instance.new("ImageLabel", FakeParryData.Button)
    logo.Name = "FakeParryLogo"
    logo.Size = UDim2.new(0.72, 0, 0.72, 0)
    logo.Position = UDim2.new(0.5, 0, 0.5, 0)
    logo.AnchorPoint = Vector2.new(0.5, 0.5)
    logo.BackgroundTransparency = 1
    logo.Image = "rbxassetid://109730107051926"
...
function VD_PlayFakeParry()
    ...
    pcall(function()
        ...
        if getgenv().AREXANS_FakeParryTrack then
            pcall(function() getgenv().AREXANS_FakeParryTrack:Stop() end)
            getgenv().AREXANS_FakeParryTrack = nil
        end
        ...
        track:Play()
        getgenv().AREXANS_FakeParryTrack = track
    end)
end

if not getgenv().AREXANS_FakeParryInputConn then
    getgenv().AREXANS_FakeParryInputConn = game:GetService("UserInputService").InputBegan:Connect(function(input, gp)
        if gp then return end
...
-- ==========================================
getgenv().AREXANS_FakeGenTrack = nil
FakeGenData = {
    UI = nil,
    Button = nil,
    DragLocked = false,
...
function VD_ToggleFakeGen()
    if not VD.SURV_FakeGen then
        if getgenv().AREXANS_FakeGenTrack then
            pcall(function() getgenv().AREXANS_FakeGenTrack:Stop() end)
            getgenv().AREXANS_FakeGenTrack = nil
        end
    ...
    if getgenv().AREXANS_FakeGenTrack then
        ...
        pcall(function()
            ...
            track.Priority = Enum.AnimationPriority.Action
            track:Play()
            getgenv().AREXANS_FakeGenTrack = track
...
do -- Killer Tab
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Abyssal Burst (Abyss)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Abyssal Burst (Abyss)", Callback = function(v)
        ...
        if v then
            AREXANS_StartAbyssCooldownBypass()
        else
            AREXANS_StopAbyssCooldownBypass()
        end
    end })
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Skill (Hidden)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Skill (Hidden)", Callback = function(v)
        ...
        if v then
            pcall(AREXANS_StartHiddenCooldownBypass)
        else
            pcall(AREXANS_StopHiddenCooldownBypass)
        end
    end })
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Frenzy (Jeff)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Frenzy (Jeff)", Callback = function(v)
        ...
        if v then
            pcall(AREXANS_StartJeffCooldownBypass)
        else
            pcall(AREXANS_StopJeffCooldownBypass)
        end
    end })
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Lake Mist (Jason)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Lake Mist (Jason)", Callback = function(v)
        ...
        if v then
            pcall(AREXANS_StartSlasherCooldownBypass)
        else
            pcall(AREXANS_StopSlasherCooldownBypass)
        end
    end })
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Fake Attack (Counter Parry)", Locked = false, TextLocked = "Premium Required", Flag = "Fake Attack (Counter Parry)", Callback = function(v)
        ...
        pcall(AREXANS_ToggleFakeAttack, v)
    end })
    ...
    pcall(function()
        ...
        abilityKiller:AddButton({
            ...
            Callback = function()
                ...
                pcall(AREXANS_ApplyCustomMasked, VD.KILLER_CustomMasked)
            end
        })
        abilityKiller:AddButton({
            ...
            Callback = function()
                ...
                pcall(AREXANS_ApplyCustomMasked, mask)
            end
        })
    end)
...
do -- Fling Tab
    ...
    flingSection:AddButton({ Name = "Fling Nearest", Locked = false, TextLocked = "Premium Required", Callback = function() 
        ...
        pcall(function() AREXANS_FlingNearest() end) 
    end })
...
print("Arexans Tools Violence District v1.5.7 Loaded")
pcall(function()
    VD_Notify("Arexans Tools", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
...
function AREXANS_ApplyCustomMasked(maskName)
...
function VD_ClearSurvivorWarnings()
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local root = char and char:FindFirstChild("HumanoidRootPart")
        local warn = root and root:FindFirstChild("AREXANS_SurvivorKillerWarn")
...
function VD_UpdateSurvivorWarnings()
    ...
    local now = tick()
    if VD._WarnKillerXanst and now < VD._WarnKillerXanst then return end
    VD._WarnKillerXanst = now + 0.15
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if IsSurvivor(player) and player.Character then
            ...
            if root and hum and hum.Health > 0 then
                ...
                local warn = root:FindFirstChild("AREXANS_SurvivorKillerWarn")
                if nearest <= 60 then
                    local danger = nearest <= 40
                    if not warn then
                        warn = Instance.new("BillboardGui")
                        warn.Name = "AREXANS_SurvivorKillerWarn"
                        warn.Adornee = root
...
function VD_UpdateBypassGate()
    ...
    if VD._XanstBypassGate and tick() < VD._XanstBypassGate then return end
    VD._XanstBypassGate = tick() + 1
...
function VD_SetInvisibleNotVisual(state)
    ...
    if state then
        ...
        seat.Name = "AREXANS_InvisibleSeat"
        seat.Anchored = false
...
function VD_CreateMoonwalkButton()
    ...
    local old = parent:FindFirstChild("AREXANS_MoonwalkButton")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "AREXANS_MoonwalkButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
...
local function VD_CreateAimLockButton()
    ...
    local old = parent:FindFirstChild("AREXANS_AimLockButton")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "AREXANS_AimLockButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
    sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
RunService.RenderStepped:Connect(function()
    ...
    if AREXANS_PerfDue("aimlock_target", VD.PotatoMode and 0.08 or 0.04) then
        VD_AimLockState.CurrentTarget = VD_AimLock_GetClosest()
    end
    local targetPart = VD_AimLockState.CurrentTarget
...
function AREXANS_ScanMap()
    ...
    AREXANS_Cache.Generators = newGens
    AREXANS_Cache.Gates      = newGates
    AREXANS_Cache.Hooks      = newHooks
    AREXANS_Cache.Pallets    = newPallets
    AREXANS_Cache.Windows    = newWindows
    AREXANS_Cache.ExitPos    = exitPos
    AREXANS_Cache.ExitPart   = exitPart
    print("[AREXANS ScanMap] Generators:", #newGens, "Gates:", #newGates, "Hooks:", #newHooks, "Windows:", #newWindows)

    local root           = Root
    if root and #AREXANS_Cache.Hooks > 0 then
        local closest, closestDist = nil, math.huge
        for _, hook in ipairs(AREXANS_Cache.Hooks) do
            if hook.part then
        ...
        AREXANS_Cache.ClosestHook = closest
    end
end
...
function CreateRadarGUI()
    ...
    radarGui.Name = "ArexansHub_RadarGUI"
    ...
    titleText.Text = "ArexansHub RADAR"
...
function UpdateRadar()
    ...
    if VD.RADAR_ShowGenerator then
        for _, gen in ipairs(AREXANS_Cache.Generators or {}) do
    ...
    if VD.RADAR_ShowPallet then
        for _, pallet in ipairs(AREXANS_Cache.Pallets or {}) do
    ...
    if VD.RADAR_ShowHook then
        for _, hook in ipairs(AREXANS_Cache.Hooks or {}) do
    ...
    if VD.RADAR_ShowGate then
        for _, gate in ipairs(AREXANS_Cache.Gates or {}) do
    ...
    if VD.RADAR_ShowWindow then
        for _, window in ipairs(AREXANS_Cache.Windows or {}) do
    ...
    if VD.RADAR_ShowZombie then
        if AREXANS_WorldReg and AREXANS_WorldReg.SCPZombie then
            for model, entry in pairs(AREXANS_WorldReg.SCPZombie) do
...
function AREXANS_TeleportToPosition(pos)
...
function AREXANS_TeleportToGenerator(index)
    if not AREXANS_Cache or not AREXANS_Cache.Generators or #AREXANS_Cache.Generators == 0 then print("[Arexans Tools] Generator tidak ditemukan") return false end

    local sorted = {}
    for _, gen in ipairs(AREXANS_Cache.Generators) do
    ...
    return AREXANS_TeleportToPosition(target.gen.part.Position)
end

function AREXANS_TeleportToGate()
    if not AREXANS_Cache or not AREXANS_Cache.Gates or #AREXANS_Cache.Gates == 0 then print("[Arexans Tools] Gate tidak ditemukan") return false end
    local closest, closestDist = nil, math.huge
    for _, gate in ipairs(AREXANS_Cache.Gates) do
    ...
    return AREXANS_TeleportToPosition(closest.part.Position)
end

function AREXANS_TeleportToHook()
    if not AREXANS_Cache or not AREXANS_Cache.ClosestHook then print("[Arexans Tools] Hook tidak ditemukan") return false end
    return AREXANS_TeleportToPosition(AREXANS_Cache.ClosestHook.part.Position)
end
...
function CheckMapChange()
    ...
    if CurrentMapName ~= mapName then
        -- print("[Arexans Tools] Map berubah: " .. tostring(CurrentMapName) .. " -> " .. mapName)
        ...
        if descendant:IsA("Model") or descendant:IsA("Folder") then
            ...
            if n:find("generator") or n:find("mesin") or n:find("pallet") or n:find("window") or n:find("hook") or n:find("gate") then
                task.delay(0.5, function()
                    ...
                    if part then
                        if n:find("generator") or n:find("mesin") then table.insert(AREXANS_Cache.Generators, {model=descendant, part=part})
                        elseif n:find("pallet") then table.insert(AREXANS_Cache.Pallets, {model=descendant, part=part})
                        elseif n:find("window") then table.insert(AREXANS_Cache.Windows, {model=descendant, part=part})
                        elseif n:find("hook") then table.insert(AREXANS_Cache.Hooks, {model=descendant, part=part})
                        elseif n:find("gate") then table.insert(AREXANS_Cache.Gates, {model=descendant, part=part})
                        end
                    end
                end)
            end
...
Workspace.ChildAdded:Connect(function(child)
    ...
    if child and child.Name == "Spearprojectile" and VD.SURV_AutoDodgeSpear and GetRole() == "Survivor" then
        task.spawn(function()
            if getgenv().AREXANS_IsDodging then return end
            ...
            if dot > 0.85 then 
                if dot > 0.85 then -- Sudut cukup sempit, berbahaya!
                    getgenv().AREXANS_IsDodging = true
                    ...
                    getgenv().AREXANS_IsDodging = false
                end
            end
        end)
    end
...
local function AREXANS_AutoAttack()
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        local windowGroups = {}
        for _, win in ipairs(AREXANS_Cache.Windows or {}) do
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        -- Scan pallet dari AREXANS_Cache + langsung dari workspace
        ...
        -- Iterasi AREXANS_Cache.Pallets untuk cari Palletwrong terdekat
        for _, pal in ipairs(AREXANS_Cache.Pallets or {}) do
...
local function AREXANS_UpdateHitboxes()
...
local function AREXANS_DestroyAllPallets()
...
-- =====================================================
getgenv().AREXANS_IsBreakingGenerator = false

function AREXANS_AutoBreakGene()
    if not VD.KILLER_AutoBreakGene or GetRole() ~= "Killer" then return end
    if getgenv().AREXANS_IsBreakingGenerator then return end
    ...
    if nearest then
        getgenv().AREXANS_IsBreakingGenerator = true
        task.spawn(function()
            pcall(function()
                local r = ReplicatedStorage:FindFirstChild("Remotes")
...
-- =====================================================
getgenv().AREXANS_LastVaultBlockTime = 0

function AREXANS_BlockAllVaults()
    if not VD.KILLER_BlockVaults or GetRole() ~= "Killer" then return end
    
    local now = tick()
    if now - getgenv().AREXANS_LastVaultBlockTime < 1.5 then return end
    getgenv().AREXANS_LastVaultBlockTime = now
    ...
    pcall(function()
        ...
        if vaultsFolder then
            ...
            -- Fallback using AREXANS_Cache.Windows if Map structure is not default
            for _, win in ipairs(AREXANS_Cache.Windows or {}) do
...
-- =====================================================
getgenv().AREXANS_LastPalletBlockTime = 0

function AREXANS_BlockAllPalletDrops()
    if not VD.KILLER_BlockPallets or GetRole() ~= "Killer" then return end

    local now = tick()
    if now - getgenv().AREXANS_LastPalletBlockTime < 2 then return end
    getgenv().AREXANS_LastPalletBlockTime = now
    ...
    pcall(function()
        ...
        -- Fallback: gunakan AREXANS_Cache.Pallets
        for _, pal in ipairs(AREXANS_Cache.Pallets or {}) do
...
-- =====================================================
getgenv().AREXANS_LastPalletBlockDropTime = 0
getgenv().AREXANS_IsBlockingPallets = false

-- Helper: force bersihkan semua attribute yang bisa bikin stuck
local function AREXANS_ForceUnstuck(char)
    pcall(function()
...
function AREXANS_BlockPalletDrop()
    if not VD.KILLER_BlockPalletDrop or GetRole() ~= "Killer" then return end
    if getgenv().AREXANS_IsBlockingPallets then return end

    local now = tick()
    if now - getgenv().AREXANS_LastPalletBlockDropTime < 4 then return end
    getgenv().AREXANS_LastPalletBlockDropTime = now
    ...
    pcall(function()
        ...
        getgenv().AREXANS_IsBlockingPallets = true
...
-- =====================================================
getgenv().AREXANS_AbyssCooldownBypassConnection = nil
getgenv().AREXANS_CorruptHandlerFunc = nil

function AREXANS_StartAbyssCooldownBypass()
    if not getgenv().AREXANS_CorruptHandlerFunc then
        for _, v in pairs(getgc(true)) do
            if type(v) == "function" and islclosure(v) then
                local constants = debug.getconstants(v)
                if table.find(constants, "corrupt") and table.find(constants, "Immobile") then
                    getgenv().AREXANS_CorruptHandlerFunc = v
                    break
                end
            end
        end
    ...
    if getgenv().AREXANS_AbyssCooldownBypassConnection then 
        getgenv().AREXANS_AbyssCooldownBypassConnection:Disconnect() 
    end
    
    getgenv().AREXANS_AbyssCooldownBypassConnection = RunService.Heartbeat:Connect(function()
        if not VD.KILLER_BypassCooldown then return end
        if not AREXANS_PerfDue("abyss_bypass", VD.PotatoMode and 0.12 or 0.06) then return end
        if getgenv().AREXANS_CorruptHandlerFunc then
            local upvalues = debug.getupvalues(getgenv().AREXANS_CorruptHandlerFunc)
            for idx, val in pairs(upvalues) do
                if type(val) == "boolean" and val == false then
                    debug.setupvalue(getgenv().AREXANS_CorruptHandlerFunc, idx, true)
                end
            end
        end
    end)
...
function AREXANS_StopAbyssCooldownBypass()
    if getgenv().AREXANS_AbyssCooldownBypassConnection then
        getgenv().AREXANS_AbyssCooldownBypassConnection:Disconnect()
        getgenv().AREXANS_AbyssCooldownBypassConnection = nil
    end
...
-- =====================================================
getgenv().AREXANS_JeffCooldownBypassThread = nil

function AREXANS_StartJeffCooldownBypass()
    if getgenv().AREXANS_JeffCooldownBypassThread then return end
    getgenv().AREXANS_JeffCooldownBypassThread = task.spawn(function()
...
function AREXANS_StopJeffCooldownBypass()
    pcall(function()
        local player = game:GetService("Players").LocalPlayer
        local char = player.Character
        if char and char:GetAttribute("Frenzy") == true then
...
getgenv().AREXANS_SlasherCooldownBypassThread = nil

function AREXANS_StartSlasherCooldownBypass()
    if getgenv().AREXANS_SlasherCooldownBypassThread then return end
    ...
    getgenv().AREXANS_SlasherCooldownBypassThread = task.spawn(function()
...
function AREXANS_StopSlasherCooldownBypass()
...
-- ==============================================
getgenv().AREXANS_FakeAttackThread = nil
function AREXANS_ToggleFakeAttack(enabled)
    if not enabled then
        if getgenv().AREXANS_FakeAttackThread then
            task.cancel(getgenv().AREXANS_FakeAttackThread)
            getgenv().AREXANS_FakeAttackThread = nil
        end
        return
    end
    
    if getgenv().AREXANS_FakeAttackThread then return end
    
    getgenv().AREXANS_FakeAttackThread = task.spawn(function()
        local Players = game:GetService("Players")
        local LocalPlayer = Players.LocalPlayer
...
function AREXANS_UpdateCureFlaskLaser()
    ...
    if originPos and targetPos and actionActive then
        if not getgenv().AREXANS_CureFlaskLaserPart then
            local laser = Instance.new("Part")
            ...
            laser.Transparency = 0
            laser.Parent = workspace
            getgenv().AREXANS_CureFlaskLaserPart = laser
        end
        
        local dist = (targetPos - originPos).Magnitude
        if dist > 0.1 then
            local laser = getgenv().AREXANS_CureFlaskLaserPart
            laser.Size = Vector3.new(0.16, 0.16, dist)
            laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
            laser.Transparency = 0
        end
        ...
        if getgenv().AREXANS_CureFlaskLaserPart then
            getgenv().AREXANS_CureFlaskLaserPart.Transparency = 1
        end
    end
end

function AREXANS_StartCureFlaskLaser()
    if getgenv().AREXANS_CureFlaskLaserThread then return end
    getgenv().AREXANS_CureFlaskLaserThread = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        pcall(AREXANS_UpdateCureFlaskLaser)
    end)
end
...
-- =====================================================
getgenv().AREXANS_HiddenLeapBypassThread = nil

function AREXANS_StartHiddenCooldownBypass()
    if getgenv().AREXANS_HiddenLeapBypassThread then return end
    getgenv().AREXANS_HiddenLeapBypassThread = task.spawn(function()
...
function AREXANS_StopHiddenCooldownBypass()
...
-- =====================================================
local function AREXANS_BeatGameSurvivor()
    ...
    if not exitPos and map then
        pcall(function()
            ...
            if AREXANS_Cache and AREXANS_Cache.ExitPos then
                exitPos = AREXANS_Cache.ExitPos
                finishPart = AREXANS_Cache.ExitPart
                return
            end
        end)
    end
...
-- =====================================================
local function AREXANS_BeatGameKiller()
...
local function AREXANS_AutoHook()
    ...
    if isCarrying then
        ...
        for _, h in ipairs(AREXANS_Cache.Hooks or {}) do
            if h.part then
        ...
        if player ~= LocalPlayer and IsSurvivor(player) and player.Character then
            ...
            if tr and hum then
                ...
                if pct <= 0.25 and pct > 0 then
                    ...
                    if AREXANS_Cache and AREXANS_Cache.Hooks then
                        for _, hh in ipairs(AREXANS_Cache.Hooks) do
...
task.spawn(function()
    while not VD.Destroyed do
        if Root and AREXANS_Cache.Hooks and #AREXANS_Cache.Hooks > 0 then
            local closest, closestDist = nil, math.huge
            for _, hook in ipairs(AREXANS_Cache.Hooks) do
                if hook.part then
                    local d = (hook.part.Position - Root.Position).Magnitude
...
task.spawn(function()
    while not VD.Destroyed do
        pcall(AREXANS_AutoAttack)
        pcall(AREXANS_UpdateHitboxes)
        pcall(AREXANS_DestroyAllPallets)
        pcall(AREXANS_AutoBreakGene)
        pcall(AREXANS_BlockAllVaults)
        pcall(AREXANS_BlockAllPalletDrops)
        pcall(AREXANS_BlockPalletDrop)

        pcall(AREXANS_BeatGameSurvivor)
        pcall(AREXANS_BeatGameKiller)
        pcall(AREXANS_AutoHook)
        task.wait(0.18)
    end
end)
...
local function AREXANS_IsTouchOnAttackButton(input)
    local pos = input.Position
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if VD.AIM_Enabled and VD.AIM_UseRMB then
        ...
        elseif input.UserInputType == Enum.UserInputType.Touch and not gpe then
            ...
            if AREXANS_IsTouchOnAttackButton(input) then
                State.AimHolding = true
            end
        end
    end
...
do -- Teleport Tab
    ...
    tpMapSection:AddButton({ Name = "TP to Gen", Callback = function() pcall(function() AREXANS_TeleportToGenerator(1) end) end })
    tpMapSection:AddButton({ Name = "TP to Gate", Callback = function() pcall(AREXANS_TeleportToGate) end })
    tpMapSection:AddButton({ Name = "TP to Hook", Callback = function() pcall(AREXANS_TeleportToHook) end })
end
...
-- =====================================================
-- AREXANST KILLER DISPLAY
-- =====================================================
getgenv().AREXANS_XanstKillerGui = nil
getgenv().AREXANS_XanstKillerRunning = false

function SetupXanstKillerIndicator()
    if getgenv().AREXANS_XanstKillerGui then pcall(function() getgenv().AREXANS_XanstKillerGui:Destroy() end) end
    getgenv().AREXANS_XanstKillerGui = nil
    AREXANS_SetMainInfoPanelText("XanstKiller", "Xanst Killer Display", "Waiting...")
end

function StartXanstKiller()
    if getgenv().AREXANS_XanstKillerRunning then return end
    getgenv().AREXANS_XanstKillerRunning = true
    SetupXanstKillerIndicator()
    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_XanstKiller and _genv.AREXANS_XanstKillerRunning do
            ...
            if nk then
                AREXANS_SetMainInfoPanelText("XanstKiller", "Xanst Killer Display", "Xanst Killer: " .. (nk == LocalPlayer and "YOU" or nk.DisplayName or nk.Name))
            else
                AREXANS_SetMainInfoPanelText("XanstKiller", "Xanst Killer Display", "Xanst Killer: None")
            end
            task.wait(2)
        end
    end)
...
function StopXanstKiller()
    getgenv().AREXANS_XanstKillerRunning = false
    if getgenv().AREXANS_XanstKillerGui then
        pcall(function() getgenv().AREXANS_XanstKillerGui:Destroy() end)
        getgenv().AREXANS_XanstKillerGui = nil
    end
    AREXANS_SetMainInfoPanelText("XanstKiller", "Xanst Killer Display", "Off")
end

-- =====================================================
-- SPECTATOR COUNTER
-- =====================================================
if getgenv().AREXANS_SpectatorCounterGui then
    pcall(function() getgenv().AREXANS_SpectatorCounterGui:Destroy() end)
end
getgenv().AREXANS_SpectatorCounterGui = nil
getgenv().AREXANS_SpectatorCounterRunning = false

function SetupSpectatorCounter()
    if getgenv().AREXANS_SpectatorCounterGui then pcall(function() getgenv().AREXANS_SpectatorCounterGui:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "AREXANS_SpectatorCounterGui"
    sg.ResetOnSpawn = false
    ...
    local pg = GetSafeGuiParent()
    if pg then
        local oldGui = pg:FindFirstChild("AREXANS_SpectatorCounterGui")
    ...
    getgenv().AREXANS_SpectatorCounterGui = sg
end

function StartSpectatorCounter()
    if getgenv().AREXANS_SpectatorCounterRunning then return end
    getgenv().AREXANS_SpectatorCounterRunning = true
    SetupSpectatorCounter()

    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_SpectatorCounter and _genv.AREXANS_SpectatorCounterRunning and getgenv().AREXANS_SpectatorCounterGui do
            local sg = getgenv().AREXANS_SpectatorCounterGui
...
if getgenv().AREXANS_KillerPerksGui then
    pcall(function() getgenv().AREXANS_KillerPerksGui:Destroy() end)
end
getgenv().AREXANS_KillerPerksGui = nil

local AREXANS_KillerPerksCollapsed = false
local AREXANS_KillerPerksLocked = false
getgenv().AREXANS_KillerPerksRunning = false

local AREXANS_KillerPerkNames = {
    XanstInLine = "Xanst in Line",
    ["Xanst in Line"] = "Xanst in Line",
...
function AREXANS_EscapeRichText(text)
    text = tostring(text or "")
    text = text:gsub("&", "&amp;")
    text = text:gsub("<", "&lt;")
    text = text:gsub(">", "&gt;")
...
local function AREXANS_FormatPerkName(name)
    name = tostring(name or "")
    if AREXANS_KillerPerkNames[name] then return AREXANS_KillerPerkNames[name] end
...
local function AREXANS_GetKillerPlayer()
    for _, player in ipairs(Players:GetPlayers()) do
...
local function AREXANS_AddWorkspacePerk(result, seen, rawName, displayName, level)
    ...
    table.insert(result, {
        Raw = rawName,
        Name = displayName and tostring(displayName) or AREXANS_FormatPerkName(rawName),
        Level = level and tostring(level) or nil,
    })
end

local function AREXANS_IsPerkContainer(inst)
...
local function AREXANS_ParseWorkspacePerkName(name)
    name = tostring(name or "")
...
local function AREXANS_ReadPerksFromWorkspace(killer)
        ...
        for key, value in pairs(attrs) do
            ...
            if lowerKey:find("perk") then
                if type(value) == "string" then
                    AREXANS_AddWorkspacePerk(result, seen, value)
                elseif value == true then
                    AREXANS_AddWorkspacePerk(result, seen, key)
                elseif type(value) == "number" and lowerKey:find("level") then
                    local baseName = tostring(key):gsub("[Ll]evel", ""):gsub("[Pp]erk", "")
                    if baseName ~= "" then
                        AREXANS_AddWorkspacePerk(result, seen, baseName, nil, value)
                    end
                end
            end
        end
    ...
    for _, child in ipairs(char:GetChildren()) do
        local perkName, level = AREXANS_ParseWorkspacePerkName(child.Name)
        if perkName then
            AREXANS_AddWorkspacePerk(result, seen, child.Name, perkName, level)
        end
    end
    ...
    for _, inst in ipairs(char:GetDescendants()) do
        ...
        if AREXANS_IsPerkContainer(inst) then
            for _, child in ipairs(inst:GetChildren()) do
                local value, level = readValueObject(child)
                AREXANS_AddWorkspacePerk(result, seen, value or child.Name, nil, level)
            end
        else
            local lowerName = inst.Name:lower()
            if lowerName:find("perk") then
                local value, level = readValueObject(inst)
                AREXANS_AddWorkspacePerk(result, seen, value or inst.Name, nil, level)
            end
        end
    end
...
local function AREXANS_BuildKillerPerksText()
    local killer = AREXANS_GetKillerPlayer()
    local killerName = killer and (killer.DisplayName or killer.Name) or "Unknown"
    local perks = AREXANS_ReadPerksFromWorkspace(killer)

    if #perks == 0 then
        for _, player in ipairs(Players:GetPlayers()) do
            local candidatePerks = AREXANS_ReadPerksFromWorkspace(player)
    ...
    local lines = {
        'Killer Perks [<font color="rgb(255,80,80)">' .. AREXANS_EscapeRichText(killerName) .. '</font>]',
    }
    ...
    if #perks == 0 then
        ...
        for i = 1, math.min(#perks, 4) do
            ...
            table.insert(lines, '<font color="rgb(255,204,80)">- ' .. AREXANS_EscapeRichText(perk.Name) .. AREXANS_EscapeRichText(levelText) .. '</font>')
        end
    end
...
local function SetupKillerPerksDisplay()
    if getgenv().AREXANS_KillerPerksGui then pcall(function() getgenv().AREXANS_KillerPerksGui:Destroy() end) end
    getgenv().AREXANS_KillerPerksGui = nil
    AREXANS_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", "Waiting for perk data...")
end

function StartKillerPerksDisplay()
    if getgenv().AREXANS_KillerPerksRunning then return end
    getgenv().AREXANS_KillerPerksRunning = true
    SetupKillerPerksDisplay()

    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_KillerPerks and _genv.AREXANS_KillerPerksRunning do
            local text = AREXANS_BuildKillerPerksText()
            AREXANS_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", text)
            task.wait(1)
        end
    end)
end
...
function StopKillerPerksDisplay()
    ...
    AREXANS_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", "Off")
end
...
if getgenv().AREXANS_PredictMapGui then
    pcall(function() getgenv().AREXANS_PredictMapGui:Destroy() end)
end
if getgenv().AREXANS_PredictMapConnections then
    for _, conn in ipairs(getgenv().AREXANS_PredictMapConnections) do
        pcall(function() conn:Disconnect() end)
    end
end
getgenv().AREXANS_PredictMapGui = nil
getgenv().AREXANS_PredictMapConnections = {}
getgenv().AREXANS_PredictMapRunning = false

local AREXANS_PredictMapState = {
    Name = "Unknown",
...
local function AREXANS_MapStringFromValue(value)
    if type(value) == "string" then
        return value
...
local function AREXANS_MapDescFromValue(value)
    if type(value) ~= "table" then return nil end
    return value.Desc or value.desc or value.Description or value.description
end

local function AREXANS_SetPredictedMap(name, desc, source)
    name = AREXANS_MapStringFromValue(name) or name
    if not name or tostring(name) == "" then return end
    AREXANS_PredictMapState.Name = tostring(name)
    if desc and tostring(desc) ~= "" then
        AREXANS_PredictMapState.Desc = tostring(desc)
    end
    AREXANS_PredictMapState.Source = source or "Detected"
end

local function AREXANS_ReadCurrentWorkspaceMap()
    local map = Workspace:FindFirstChild("Map")
...
local function AREXANS_TryPredictMapRemote()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    for _, args in ipairs(attempts) do
        ...
        if ok and data ~= nil then
            local name = AREXANS_MapStringFromValue(data)
            local desc = AREXANS_MapDescFromValue(data)
            if name then
                AREXANS_SetPredictedMap(name, desc, "Predicted")
                return true
            end
        end
    end
...
local function AREXANS_PredictMapText()
    local name = AREXANS_EscapeRichText(AREXANS_PredictMapState.Name or "Unknown")
    local source = AREXANS_EscapeRichText(AREXANS_PredictMapState.Source or "Idle")
    local phase = tostring(AREXANS_PredictMapState.Phase or "")
    local timeLeft = AREXANS_PredictMapState.TimeLeft
    local timerText = ""
    if timeLeft then
...
local function SetupPredictMapGui()
    if getgenv().AREXANS_PredictMapGui then pcall(function() getgenv().AREXANS_PredictMapGui:Destroy() end) end
    getgenv().AREXANS_PredictMapGui = nil
    AREXANS_SetMainInfoPanelText("PredictMap", "Predict Map", AREXANS_PredictMapText())
end

local function AREXANS_BindPredictMapEvents()
    local conns = getgenv().AREXANS_PredictMapConnections
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    if mapInfo and mapInfo:IsA("RemoteEvent") then
        table.insert(conns, mapInfo.OnClientEvent:Connect(function(a, b, c)
            local name = AREXANS_MapStringFromValue(a) or AREXANS_MapStringFromValue(b) or AREXANS_MapStringFromValue(c)
            local desc = AREXANS_MapDescFromValue(a) or AREXANS_MapDescFromValue(b) or AREXANS_MapDescFromValue(c)
            if name then
                AREXANS_SetPredictedMap(name, desc, "Predicted")
            end
        end))
    ...
    if timeEvent and timeEvent:IsA("RemoteEvent") then
        table.insert(conns, timeEvent.OnClientEvent:Connect(function(phase, timeLeft)
            AREXANS_PredictMapState.Phase = tostring(phase or "")
            AREXANS_PredictMapState.TimeLeft = tonumber(timeLeft)
            if tostring(phase) == "Intermission" and tonumber(timeLeft) and tonumber(timeLeft) <= 20 then
                pcall(AREXANS_TryPredictMapRemote)
            end
        end))
    end
    ...
    table.insert(conns, Workspace.ChildAdded:Connect(function(child)
        if child and child.Name == "Map" then
            task.delay(0.25, function()
                local name, desc = AREXANS_ReadCurrentWorkspaceMap()
                if name then AREXANS_SetPredictedMap(name, desc, "Confirmed") end
            end)
        end
    end))
end
...
function StartPredictMap()
    if getgenv().AREXANS_PredictMapRunning then return end
    getgenv().AREXANS_PredictMapRunning = true
    SetupPredictMapGui()
    AREXANS_BindPredictMapEvents()

    task.spawn(function()
        while VD and VD.VIS_PredictMap and getgenv().AREXANS_PredictMapRunning do
            local currentName, currentDesc = AREXANS_ReadCurrentWorkspaceMap()
            if currentName then
                AREXANS_SetPredictedMap(currentName, currentDesc, "Confirmed")
            else
                pcall(AREXANS_TryPredictMapRemote)
            end
...
function StopPredictMap()
    ...
    getgenv().AREXANS_PredictMapConnections = {}
    AREXANS_SetMainInfoPanelText("PredictMap", "Predict Map", "Off")
end
...
-- =====================================================
getgenv().AREXANS_OriginalFOV          = nil
getgenv().AREXANS_OriginalCameraType   = nil
getgenv().AREXANS_OriginalCameraOffset = nil
getgenv().AREXANS_ThirdPersonWasActive = false
getgenv().AREXANS_FOVWasActive         = false
...
function UpdateCameraFOV()
    ...
    if VD.CAM_FOVEnabled then
        if not getgenv().AREXANS_FOVWasActive then
            getgenv().AREXANS_OriginalFOV = cam.FieldOfView
            getgenv().AREXANS_FOVWasActive = true
        end
        cam.FieldOfView = VD.CAM_FOV or 90
    elseif getgenv().AREXANS_FOVWasActive then
        if getgenv().AREXANS_OriginalFOV then cam.FieldOfView = getgenv().AREXANS_OriginalFOV end
        getgenv().AREXANS_OriginalFOV = nil
        getgenv().AREXANS_FOVWasActive = false
    end
end
...
function UpdateThirdPerson()
    ...
    if shouldBeActive then
        if not getgenv().AREXANS_ThirdPersonWasActive then
            getgenv().AREXANS_OriginalCameraType = cam.CameraType
            local char = LocalPlayer.Character
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            getgenv().AREXANS_OriginalCameraOffset = hum and hum.CameraOffset or Vector3.new(0, 0, 0)
        end
        cam.CameraType = Enum.CameraType.Custom
        local char     = LocalPlayer.Character
        local hum      = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.CameraOffset = Vector3.new(2, 1, 8) end
        getgenv().AREXANS_ThirdPersonWasActive = true
    elseif getgenv().AREXANS_ThirdPersonWasActive then
        if getgenv().AREXANS_OriginalCameraType then
            cam.CameraType = getgenv().AREXANS_OriginalCameraType; getgenv().AREXANS_OriginalCameraType = nil
        end
        local char = LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.CameraOffset = getgenv().AREXANS_OriginalCameraOffset or Vector3.new(0, 0, 0) end
        getgenv().AREXANS_OriginalCameraOffset = nil
        getgenv().AREXANS_ThirdPersonWasActive = false
    end
end

getgenv().AREXANS_shiftLockWasActive = false
...
function UpdateShiftLock()
    ...
    if VD.CAM_ShiftLock then
        ...
        getgenv().AREXANS_shiftLockWasActive = true
        ...
        if getgenv().AREXANS_shiftLockWasActive then
        ...
        if map then
            for _, obj in ipairs(map:GetDescendants()) do
                if obj.Name:lower():find("fog") or obj:IsA("Atmosphere") or obj:IsA("BloomEffect") or obj:IsA("BlurEffect") or obj:IsA("ColorCorrectionEffect") then
                    if not getgenv().AREXANS_FogCache[obj] then
                        getgenv().AREXANS_FogCache[obj] = {
    ...
    pcall(function()
        ...
        for _, obj in ipairs(lt:GetChildren()) do
            if obj:IsA("Atmosphere") or obj.Name:lower():find("fog") then
                if not getgenv().AREXANS_FogCache[obj] then getgenv().AREXANS_FogCache[obj] = { enabled = true, parent = obj.Parent } end
...
function RestoreFog()
    pcall(function()
        for obj, data in pairs(getgenv().AREXANS_FogCache) do
...
function SetupAntiStunSlowdown()
    if getgenv().AREXANS_AntiStunHooked then return end
    getgenv().AREXANS_AntiStunHooked = true
...
function SetupNoCutsceneHook()
    if getgenv().AREXANS_NoCutsceneHooked then return end
    getgenv().AREXANS_NoCutsceneHooked = true
...
function AREXANS_SetupNoCutsceneListeners()
    task.spawn(function()
...
task.spawn(AREXANS_SetupNoCutsceneListeners)
...
-- =====================================================
getgenv().AREXANS_MobileGui = getgenv().AREXANS_MobileGui or { AimBtn=nil, FOVFrame=nil, FOVStroke=nil }
...
function CreateMobileUI()
    ...
    fovStk.Parent = fovF
    getgenv().AREXANS_MobileGui.FOVFrame = fovF; getgenv().AREXANS_MobileGui.FOVStroke = fovStk

    -- === AIMBOT BUTTON (ScreenGui terpisah agar AlwaysOnTop) ===
    local aimSG = Instance.new("ScreenGui")
    aimSG.Name           = "AREXANS_AimBtn"
    ...
    getgenv().AREXANS_MobileGui.AimBtn = btn
end
...
function setupSpearAimbotBtn()
    ...
    print("[ArexansHub] Spear Aimbot Button created successfully")
end
...
function UpdateMobileFOV()
    if not getgenv().AREXANS_MobileGui.FOVFrame then return end
    if VD.AIM_Enabled and VD.AIM_ShowFOV then
        local r = (VD.AIM_FOV or 120)
        getgenv().AREXANS_MobileGui.FOVFrame.Size = UDim2.new(0, r*2, 0, r*2)
        getgenv().AREXANS_MobileGui.FOVStroke.Color = State.AimTarget and Color3.fromRGB(90,220,120) or Color3.fromRGB(220,70,70)
        getgenv().AREXANS_MobileGui.FOVFrame.Visible = true
    else
        getgenv().AREXANS_MobileGui.FOVFrame.Visible = false
    end
end
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    -- Frame-sensitive/mobile camera work stays smooth.
    if not DrawingAvailable then
        UpdateCameraFOV()
        UpdateThirdPerson()
        UpdateShiftLock()
        if VD.SPEAR_Aimbot and AREXANS_PerfDue("mobile_spear_aim", VD.PotatoMode and 0.06 or 0.03) then
            pcall(UpdateSpearAim)
        end
    end
    if not DrawingAvailable and VD.AIM_Enabled and State.AimHolding and AREXANS_PerfDue("mobile_aim", VD.PotatoMode and 0.06 or 0.03) then
        local sc = cam.ViewportSize
        pcall(function() Aimbot.Update(cam, sc, Vector2.new(sc.X/2, sc.Y/2)) end)
    end

    -- UI maintenance is throttled; creating UI every Heartbeat can cause frame spikes.
    if not DrawingAvailable and AREXANS_PerfDue("mobile_ui", 0.25) then
        if not getgenv().AREXANS_MobileGui.FOVFrame or not getgenv().AREXANS_MobileGui.FOVFrame.Parent then
            pcall(CreateMobileUI)
        end
        if getgenv().AREXANS_MobileGui.AimBtn then
            getgenv().AREXANS_MobileGui.AimBtn.Visible = VD.AIM_Enabled
        end
        pcall(UpdateMobileFOV)
    end

    if AREXANS_PerfDue("radar", VD.PotatoMode and 0.16 or 0.08) then
        pcall(UpdateRadar)
    end
    if AREXANS_PerfDue("survivor_warnings", VD.PotatoMode and 0.18 or 0.10) then
        pcall(VD_UpdateSurvivorWarnings)
    end
    if AREXANS_PerfDue("game_utilities", VD.PotatoMode and 0.16 or 0.08) then
    ...
    if AREXANS_PerfDue("weather_anchor", 0.08) then
        pcall(VD_UpdateWeatherAnchor)
    end
    -- Moonwalk remains frequent enough for smooth movement but is no longer
    -- mixed with all the expensive visual/game scanners above.
    pcall(VD_UpdateMoonwalk, deltaTime)

    if SpearBtnData and SpearBtnData.Button and AREXANS_PerfDue("spear_button", 0.25) then
...
-- =====================================================
getgenv().AREXANS_SyncLoadedFeatures = function()
    ...
    -- Sync Xanst Killer Display
    if VD.VIS_XanstKiller then
        pcall(StartXanstKiller)
    else
        pcall(StopXanstKiller)
    end
    ...
    if getgenv().AREXANS_SetHideSurvivorIcon then
        pcall(getgenv().AREXANS_SetHideSurvivorIcon, VD.VIS_HideSurvivorIcon)
    end

    -- Sync Ping & FPS Display
    if getgenv().AREXANS_SetShowPingFPS then
        pcall(getgenv().AREXANS_SetShowPingFPS, VD.VIS_ShowPingFPS)
    end

    -- Sync Hook Counter
    if getgenv().AREXANS_SetShowHookCounter then
        pcall(getgenv().AREXANS_SetShowHookCounter, VD.VIS_ShowHookCounter)
    end

    -- Sync Twist of Fate silent aim
    if getgenv().AREXANS_SetToFSilentAim then
        pcall(getgenv().AREXANS_SetToFSilentAim, VD.TOF_SilentAim)
    end

    -- Sync Flashlight silent aim
    if getgenv().AREXANS_SetFlashlightSilentAim then
        pcall(getgenv().AREXANS_SetFlashlightSilentAim, VD.FLASH_SilentAim)
    end
...
getgenv().AREXANS_SyncUILibraryConfigRuntime = function()
    local tofValue = readConfigElementValue("Silent Aim Twist Of Fate")
    if type(tofValue) == "boolean" and tofValue ~= VD.TOF_SilentAim and getgenv().AREXANS_SetToFSilentAim then
        pcall(getgenv().AREXANS_SetToFSilentAim, tofValue)
    elseif type(tofValue) == "boolean" then
        VD.TOF_SilentAim = tofValue
        if tofValue and getgenv().AREXANS_SetToFSilentAim then
            pcall(getgenv().AREXANS_SetToFSilentAim, true)
        end
    end

    local flashValue = readConfigElementValue("Silent Aim Flashlight")
    if type(flashValue) == "boolean" and flashValue ~= VD.FLASH_SilentAim and getgenv().AREXANS_SetFlashlightSilentAim then
        pcall(getgenv().AREXANS_SetFlashlightSilentAim, flashValue)
    elseif type(flashValue) == "boolean" then
        VD.FLASH_SilentAim = flashValue
        if flashValue and getgenv().AREXANS_SetFlashlightSilentAim then
            pcall(getgenv().AREXANS_SetFlashlightSilentAim, true)
        end
    end
end
